...
test("report diagnostics snapshot includes explicit page and type-score verification row", () => {
  ...
  assert.match(
    html,
    /id="extractedVerificationValue"[^>]*>Detected pages: Not available · Type scores populated: 0\/9 · Python mismatches: 0</,
...
test("data quality diagnostics summary includes detected pages and type score coverage", () => {
  ...
  assert.match(
    script,
    /Python verification:\s*\$\{[^}]+\}/,
    "Expected diagnostics summary to include Python cross-check status",
  );
  assert.match(
    script,
