...
test("iEQ9 extraction script prompt includes expert section/page targeting guidance", () => {
  const source = loadScript();
  ...
  assert.match(
    ...
    "Expected extraction prompt to include explicit enneagram-expert parsing guidance.",
  );
  assert.match(source, /strain_interpretation\s*\(pages 20,\s*21,\s*22\)/i);
  assert.match(source, /body_language\s*\(page 25\)/i);
  assert.match(source, /feedback_guide\s*\(pages 28,\s*29\)/i);
  assert.match(source, /decision_framework\s*\(pages 32,\s*33,\s*34\)/i);
  assert.match(source, /strategic_leadership\s*\(pages 37,\s*38\)/i);
  assert.match(source, /team_dynamics\s*\(pages 39,\s*40,\s*41\)/i);
  assert.match(source, /coaching_relationship\s*\(page 42\)/i);
  assert.match(source, /development_exercises\s*\(pages 7,\s*11,\s*13,\s*17,\s*19,\s*31,\s*36,\s*38\)/i);
});
