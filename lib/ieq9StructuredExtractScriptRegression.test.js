...
test("iEQ9 shared extraction config defines canonical section page map", () => {
  ...
  assert.deepEqual(config?.section_page_map?.strain_interpretation, [18, 19, 20, 21, 22]);
...
test("parsePdf runtime imports shared iEQ9 targeted extraction config", () => {
  const source = readFileSync(path.join(process.cwd(), "lib", "parsePdf.js"), "utf8");
  assert.match(source, /ieq9_targeted_extraction_config\.json/);
});
