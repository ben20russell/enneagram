...
async function runSectionsPass({ endpoint, apiKey, apiVersion, deployment, pdfDataUrl }) {
  return runResponsesPass({
    ...
    userPrompt:
      "Return section-by-section extraction for the full report. Include PRO keys strategic_leadership, team_dynamics, coaching_relationship, and feedback_matrix when present. For pages 28-29 feedback guide, treat it as a visual grid with icons and extract text adjacent to numbers 1 through 9.",
...
function normalizeProSections(proSectionsRaw) {
  ...
  const proSectionDefinitions = [
    { id: "strategic_leadership", title: "Strategic Leadership" },
    { id: "team_dynamics", title: "Team Dynamics" },
    { id: "coaching_relationship", title: "Coaching Relationship" },
    { id: "feedback_matrix", title: "Feedback Matrix" },
  ];

  return proSectionDefinitions
    .map((definition) => {
      const raw = proSectionsRaw?.[definition.id];
      ...
      const sectionTitle = normalizePdfLayerText(raw?.sectionTitle || definition.title) || definition.title;
      return {
...
function mergeProSectionsIntoSections(sections, proSectionsRaw) {
  ...
  const keys = new Set(baseSections.map((section) => buildSectionIdentity(section)));
...
async function runSectionsFromPagesViaChat({ endpoint, apiKey, apiVersion, deployment, pagesPayload }) {
  ...
  const userPrompt = [
    ...
    "Use snake_case section ids. Include strategic_leadership, team_dynamics, coaching_relationship, and feedback_matrix for PRO reports when present.",
