...
const REPAIRED_HTML_SINGLE_PASS_MAX_CHARS = 26_000;
const REPAIRED_HTML_SEGMENT_MAX_CHARS = 14_000;
const REPAIRED_HTML_SEGMENT_OVERLAP_CHARS = 1_100;
const REPAIRED_HTML_MIN_SEGMENT_CHARS = 700;
...
const ATTACHED_JSON_SEMANTIC_EXTRACTION_GUIDANCE =
  "Extract data based on semantic alignment. If looking for a section like 'Focus of Attention', capture the full conceptual text block even if the exact section header is embedded inside HTML layout/table tags.";
const SEGMENT_MERGE_LATE_WIN_PATHS = new Set([
  "core_profile.type_number",
  "core_profile.type_name",
  "core_profile.instinctual_subtype.type",
  "core_profile.level_of_integration",
]);
...
${ATTACHED_JSON_SEMANTIC_EXTRACTION_GUIDANCE}
...
3. ${ATTACHED_JSON_SEMANTIC_EXTRACTION_GUIDANCE}
4. Normalize obvious OCR artifacts before emitting JSON:
   - Merge split words like "sur v ive", "di fficult", "Sum m ary", and "d i r e c t i o n".
   - Split merged words like "placewhere" -> "place where" and "longas" -> "long as".
   - Repair obvious line-wrap sentence fragments into complete grammatical sentences without changing facts.
   - For bullet lists, remove dangling line-wrap fragments and keep each bullet as a complete clause.
   - Fix accidental spaces around punctuation while preserving original meaning.
   - For centers_of_expression.<action|feeling|thinking>.level, normalize to HIGH, MEDIUM, or LOW when detectable.
5. Preserve table meaning exactly as represented in HTML table cells (td/th).
6. PUNCTUATION ENFORCEMENT: If original bullet points or list items lack periods, you MUST add a period at the end of each complete thought. If extracting a list into a single string field, separate distinct items with periods. Never merge separate bullet points into a run-on sentence without punctuation.
7. PUNCTUATION ENFORCEMENT: Ensure all bullet points and extracted text arrays are populated as complete grammatical sentences. If the source material lacks periods or punctuation at the end of a line, semantically complete the thought with a period so it can be split correctly downstream.
8. Do not infer from image layout; use only provided repaired text.
9. Follow this targeted extraction guidance for higher-accuracy section capture:
...
10. Output ONLY valid JSON matching this structure:
{
...
const REPAIRED_HTML_SECTION_BLUEPRINTS = [
  {
    id: "core_patterns",
    title: "Core Patterns",
    focus: "core_profile identity, core motivation/fear, and focus-of-attention narrative",
    markers: [
      "core belief & attention pattern",
      "core belief",
      "attention pattern",
      "focus of attention",
      "core motivation",
  ...
  {
    id: "instincts_subtypes",
    title: "Instincts and Subtypes",
    focus: "dominant instinct and instinctual subtype wording",
    markers: [
      "27 subtypes",
      "subtypes & instincts",
  ...
  {
    id: "centers_of_expression",
    title: "Centers of Expression",
    focus: "centers_of_expression feeling/action/thinking levels, mode, and impact",
    markers: [
      "centers of expression",
  ...
  },
  {
    ...
    focus: "overall/vocational/interpersonal/environmental/physical/psychological/happiness strain fields",
    markers: [
      "strain profile",
      "overall strain",
      "vocational strain",
  ...
  },
  {
    id: "communication",
    title: "Communication",
    focus: "communication_dynamics verbal style, language cues, listening habits, body language",
    markers: [
      "communication",
      "verbal style",
      "language cues",
  ...
  },
  {
    ...
    focus: "feedback giving and receiving guidance",
    markers: [
      "feedback guide",
  ...
  {
    id: "conflict_and_triggers",
    title: "Conflict Responses",
    focus: "conflict_and_triggers primary triggers, triggered behaviour, and response guidance",
    markers: [
      "conflict response",
  ...
  {
    id: "decision_and_leadership",
    title: "Decision and Leadership",
    focus: "decision_making and leadership_and_management guidance",
    markers: [
      "decision making",
  ...
  {
    id: "team_and_coaching",
    title: "Team and Coaching",
    focus: "team_behaviour and coaching_relationship sections",
    markers: [
      "team behaviour",
      "team behavior",
      "forming",
  ...
  {
    id: "lines_of_development",
    title: "Lines of Development",
    focus: "lines_of_development release/stretch points and wing influence",
    markers: [
...
const AGENTIC_OCR_REPAIR_SYSTEM_PROMPT =
  "You are an OCR repair agent. Fix broken font kerning and split/merged words (e.g., 'Sum m ary' -> 'Summary', 'placewhere' -> 'place where', 'longas' -> 'long as'). Repair obvious line-wrap sentence fragments while preserving meaning. In bullet lists, remove dangling fragments and ensure each bullet is a complete clause. Fix malformed HTML table tags (<table>, <tr>, <th>, <td>) without changing facts. Do NOT alter facts, names, numbers, or labels. Return only the repaired text.";

const OCR_ARTIFACT_PHRASE_REPLACEMENTS = Object.freeze([
  {
    pattern: /\b([Ss])um\s+m\s+ary\b/g,
    replacement: "summary",
  },
]);

const OCR_ARTIFACT_TOKEN_REPLACEMENTS = Object.freeze({
...
function applyMatchCasing(source, replacement) {
  const sourceText = String(source || "");
  const replacementText = String(replacement || "");
  const lettersOnly = sourceText.replace(/[^A-Za-z]+/g, "");
  if (!lettersOnly) return replacementText;
  if (lettersOnly === lettersOnly.toUpperCase()) return replacementText.toUpperCase();
...
function normalizeOcrArtifactString(value, options = {}) {
  const preserveLineBreaks = options?.preserveLineBreaks !== false;
  let normalized = sanitizePdfExtractedText(value, { preserveLineBreaks });
  if (!normalized) return normalized;

  for (const replacementEntry of OCR_ARTIFACT_PHRASE_REPLACEMENTS) {
    if (!replacementEntry?.pattern) continue;
    const replacementText = String(replacementEntry?.replacement || "").trim();
    if (!replacementText) continue;
    normalized = normalized.replace(
      replacementEntry.pattern,
      (match) => applyMatchCasing(match, replacementText),
    );
  }

  normalized = normalized.replace(/\b(?:[A-Za-z]\s+){2,}[A-Za-z]\b/g, (match) => match.replace(/\s+/g, ""));
  normalized = normalized.replace(/\b[A-Za-z]{4,}\b/g, (token) => {
    const candidate = OCR_ARTIFACT_TOKEN_REPLACEMENTS[token.toLowerCase()];
    if (!candidate) return token;
    return applyMatchCasing(token, candidate);
  });
...
function normalizeOcrArtifactPayload(value, depth = 0) {
  if (depth > 10 || value == null) return value;
  if (typeof value === "string") return normalizeOcrArtifactString(value, { preserveLineBreaks: true });
  if (Array.isArray(value)) return value.map((entry) => normalizeOcrArtifactPayload(entry, depth + 1));
  if (typeof value !== "object") return value;

  const normalizedEntries = Object.entries(value).map(([key, entry]) => [
    key,
    normalizeOcrArtifactPayload(entry, depth + 1),
  ]);
...
function extractJsonPayloadFromPythonStdout(rawStdout) {
  const normalizedStdout = String(rawStdout || "").trim();
  if (!normalizedStdout) return {};

  try {
    return JSON.parse(normalizedStdout);
  } catch {
    // Some Python extractors print parser diagnostics before JSON payload.
    // Recover by parsing the trailing JSON object from stdout.
  }

  const trailingJsonMatch = normalizedStdout.match(/\{[\s\S]*\}\s*$/);
  if (trailingJsonMatch?.[0]) {
    try {
      return JSON.parse(trailingJsonMatch[0]);
    } catch {
      // fall through to last-line parsing
    }
  }

  const trailingJsonLine = normalizedStdout
    .split(/\r?\n/)
  ...
  if (trailingJsonLine) {
    try {
      return JSON.parse(trailingJsonLine);
    } catch {
      // fall through to throw
    }
  }

  throw new Error("Python script output did not contain a valid JSON payload.");
}

function resolvePythonCommandCandidates() {
  const override = String(process.env.PDF_PYTHON_BIN || "").trim();
  const cwd = process.cwd();
  const localVenvCandidates = [];
  try {
    const cwdEntries = readdirSync(cwd, { withFileTypes: true });
    for (const entry of cwdEntries) {
      if (!entry?.isDirectory?.()) continue;
      if (!/^\.venv(?:[-_].+)?$/i.test(String(entry.name || ""))) continue;
      const venvBinDir = path.join(cwd, entry.name, "bin");
      localVenvCandidates.push(path.join(venvBinDir, "python3"));
      localVenvCandidates.push(path.join(venvBinDir, "python"));
    }
  } catch {
    // best-effort local venv discovery
  }
...
async function runPythonJsonScript({
  ...
  timeout,
  logLabel = "[parsePdf][python]",
}) {
  const pythonCandidates = resolvePythonCommandCandidates();
  const attemptErrors = [];
  ...
  for (const pythonBin of pythonCandidates) {
    try {
      ...
      if (Number.isFinite(Number(timeout)) && Number(timeout) > 0) {
        execOptions.timeout = Math.floor(Number(timeout));
      }
      const { stdout } = await execFileAsync(pythonBin, [scriptPath, ...asArray(args)], execOptions);
      const payload = extractJsonPayloadFromPythonStdout(stdout);
      return { payload, pythonBin };
    } catch (error) {
      const details = String(error?.message || error || "");
      attemptErrors.push(`${pythonBin}: ${details}`);
      const isMissingInterpreter = /ENOENT|command not found|not found/i.test(details);
      const isMissingModule = /No module named ['"]?[A-Za-z0-9_.-]+['"]?/i.test(details);
      if (isMissingInterpreter || isMissingModule) {
        console.log(`${logLabel} Python candidate unavailable; trying next candidate.`, {
          pythonBin,
          details,
        });
        continue;
      }
      console.log(`${logLabel} Python candidate failed; trying next candidate.`, {
        pythonBin,
        details,
      });
    }
  ...
  throw new Error(
    `Failed to execute Python parser script with JSON output. Attempts: ${attemptErrors.join(" | ") || "none"}`,
  );
}
...
async function extractPdfPagesWithPython(pdfBuffer) {
  ...
  try {
    ...
    const { payload } = await runPythonJsonScript({
      scriptPath: parserScriptPath,
      args: [inputPdfPath],
      maxBuffer: LOCAL_PYTHON_MAX_BUFFER_BYTES,
      logLabel: "[parsePdf][extractPdfPagesWithPython]",
    });
...
async function extractLayoutHtmlWithPython({
  ...
  try {
    ...
    const { payload, pythonBin } = await runPythonJsonScript({
      scriptPath: parserScriptPath,
      args: [inputPdfPath],
      maxBuffer: LOCAL_PYTHON_MAX_BUFFER_BYTES,
      timeout: PYTHON_VERIFICATION_TIMEOUT_MS,
      logLabel: "[parsePdf][extractLayoutHtmlWithPython]",
    });
...
async function extractDashboardVerificationWithPython({
  ...
  try {
    ...
    const { payload } = await runPythonJsonScript({
      ...
      maxBuffer: LOCAL_PYTHON_MAX_BUFFER_BYTES,
      timeout: PYTHON_VERIFICATION_TIMEOUT_MS,
      logLabel: "[parsePdf][extractDashboardVerificationWithPython]",
    });
...
function getVerificationCheckStatus(verification, fieldName) {
  const status = stringOrNull(verification?.checks?.[fieldName]?.status);
  return status ? status.toLowerCase() : null;
}

function shouldApplyResolvedValue({
  currentValue,
  resolvedValue,
  mismatchStatus,
  normalize = (value) => value,
  allowPlaceholderCurrent = false,
}) {
  const currentComparable = normalize(currentValue);
  const resolvedComparable = normalize(resolvedValue);
  const hasResolved = resolvedComparable != null && String(resolvedComparable).length > 0;
  if (!hasResolved) return false;

  const hasCurrent = currentComparable != null && String(currentComparable).length > 0;
  if (!hasCurrent) return true;

  if (allowPlaceholderCurrent && isPlaceholderTypeName(currentValue)) {
    return true;
  }

  if (mismatchStatus === "mismatch") {
    return currentComparable !== resolvedComparable;
  }
...
function applyVerificationResolvedFieldFallbacks(parsedData, verification) {
  ...
  const mismatchStatus = {
    primaryType: getVerificationCheckStatus(verification, "primaryType"),
    typeName: getVerificationCheckStatus(verification, "typeName"),
    instinctualVariant: getVerificationCheckStatus(verification, "instinctualVariant"),
    integrationLevel: getVerificationCheckStatus(verification, "integrationLevel"),
  };

  const resolvedPrimaryType = normalizeTypeNumber(resolvedFields?.primaryType);
  if (shouldApplyResolvedValue({
    currentValue: nextParsedData?.primaryType,
    resolvedValue: resolvedPrimaryType,
    mismatchStatus: mismatchStatus.primaryType,
    normalize: normalizeTypeNumber,
  })) {
    const hadTypeScores = hasAnyFiniteScore(nextParsedData?.typeScores);
    nextParsedData.primaryType = resolvedPrimaryType;
    nextParsedData.primary_type = String(resolvedPrimaryType);
    nextParsedData.core_type = resolvedPrimaryType;
    nextParsedData.typeScores = buildTypeScoresFromPrimaryType(resolvedPrimaryType);
    if (nextParsedData?.attachedProfile?.core_profile && typeof nextParsedData.attachedProfile.core_profile === "object") {
      nextParsedData.attachedProfile = {
        ...nextParsedData.attachedProfile,
        core_profile: {
          ...nextParsedData.attachedProfile.core_profile,
          type_number: resolvedPrimaryType,
        },
      };
    }
    fallbackApplied.primaryType = true;
    console.log("[parsePdf] Verification fallback updated primary type identity.", {
      resolvedPrimaryType,
      hadTypeScores,
    });
  }
  ...
  if (shouldApplyResolvedValue({
    currentValue: currentTypeName,
    resolvedValue: resolvedTypeName,
    mismatchStatus: mismatchStatus.typeName,
    normalize: normalizeTypeNameForCompare,
    allowPlaceholderCurrent: true,
  })) {
  ...
  if (shouldApplyResolvedValue({
    currentValue: nextParsedData?.instinctualVariant,
    resolvedValue: resolvedInstinctualVariant,
    mismatchStatus: mismatchStatus.instinctualVariant,
    normalize: normalizeInstinctualVariant,
  })) {
    nextParsedData.instinctualVariant = resolvedInstinctualVariant;
    nextParsedData.dominant_instinct = resolvedInstinctualVariant;
    nextParsedData.instinctScores = buildInstinctScoresFromVariant(resolvedInstinctualVariant);
    if (nextParsedData?.attachedProfile?.core_profile?.instinctual_subtype
      && typeof nextParsedData.attachedProfile.core_profile.instinctual_subtype === "object") {
      nextParsedData.attachedProfile = {
        ...nextParsedData.attachedProfile,
        core_profile: {
          ...nextParsedData.attachedProfile.core_profile,
          instinctual_subtype: {
            ...nextParsedData.attachedProfile.core_profile.instinctual_subtype,
            type: String(resolvedInstinctualVariant).toUpperCase(),
          },
        },
      };
    }
  ...
  if (shouldApplyResolvedValue({
    currentValue: nextParsedData?.integrationLevel,
    resolvedValue: resolvedIntegrationLevel,
    mismatchStatus: mismatchStatus.integrationLevel,
    normalize: normalizeIntegrationValue,
  })) {
    ...
    if (nextParsedData?.attachedProfile?.core_profile && typeof nextParsedData.attachedProfile.core_profile === "object") {
      nextParsedData.attachedProfile = {
        ...
        core_profile: {
          ...nextParsedData.attachedProfile.core_profile,
          level_of_integration: resolvedIntegrationLevel,
        },
      };
    }
  ...
  console.log("[parsePdf] Verification fallback reconciliation complete.", {
    mismatchStatus,
    fallbackApplied,
    resolvedPrimaryType,
...
function applyDeterministicRawTextFallbacks(parsedData, rawText) {
  ...
  const currentPrimaryType = normalizeTypeNumber(nextParsedData?.primaryType);
  const shouldApplyDetectedPrimaryType = detectedPrimaryType != null
    && (currentPrimaryType == null || currentPrimaryType !== detectedPrimaryType);
  if (shouldApplyDetectedPrimaryType) {
    nextParsedData.primaryType = detectedPrimaryType;
    nextParsedData.core_type = detectedPrimaryType;
    nextParsedData.typeScores = buildTypeScoresFromPrimaryType(detectedPrimaryType);
  }
  if (shouldApplyDetectedPrimaryType) {
  ...
  const currentInstinct = normalizeInstinctualVariant(nextParsedData?.instinctualVariant);
  const shouldApplyDetectedInstinct = Boolean(detectedDominantInstinct)
    && (!currentInstinct || currentInstinct !== detectedDominantInstinct);
  if (shouldApplyDetectedInstinct) {
    nextParsedData.instinctualVariant = detectedDominantInstinct;
    nextParsedData.instinctScores = buildInstinctScoresFromVariant(detectedDominantInstinct);
  }
  if (shouldApplyDetectedInstinct) {
...
function normalizeIdentitySafeguardConfig(identitySafeguard) {
  const config = identitySafeguard && typeof identitySafeguard === "object"
    ? identitySafeguard
    : null;
  if (!config) {
    return {
  ...
  const prior = config?.priorVerified && typeof config.priorVerified === "object"
    ? config.priorVerified
    : {};
  return {
    enabled: config?.enabled !== false,
    lockOnMismatch: config?.lockOnMismatch !== false,
    source: stringOrNull(config?.source) || "identity-safeguard",
    priorVerified: {
      primaryType: normalizeTypeNumber(prior?.primaryType),
      typeName: stringOrNull(prior?.typeName),
      instinctualVariant: normalizeInstinctualVariant(prior?.instinctualVariant),
      integrationLevel: normalizeIntegrationValue(prior?.integrationLevel),
    },
  };
}

function applyIdentitySafeguardToParsedData({
  parsedData,
  rawText,
  identitySafeguardConfig,
}) {
  const config = normalizeIdentitySafeguardConfig(identitySafeguardConfig);
  const nextParsedData = parsedData && typeof parsedData === "object" ? { ...parsedData } : {};
  if (!config.enabled) {
    return {
      parsedData: nextParsedData,
      ...
      resolved: {
        ...
        instinctualVariant: null,
        integrationLevel: null,
        evidencePrimaryType: null,
        evidenceInstinctualVariant: null,
      },
    };
  }

  const normalizedRawText = stringOrNull(rawText);
  const evidencePrimaryType = normalizedRawText ? extractPrimaryTypeFromRawText(normalizedRawText) : null;
  const evidenceInstinctualVariant = normalizedRawText ? extractDominantInstinctFromRawText(normalizedRawText) : null;
  const resolvedPrimaryType = evidencePrimaryType || config?.priorVerified?.primaryType || null;
  const resolvedInstinctualVariant = evidenceInstinctualVariant || config?.priorVerified?.instinctualVariant || null;
  const resolvedTypeName = stringOrNull(config?.priorVerified?.typeName);
  const resolvedIntegrationLevel = normalizeIntegrationValue(config?.priorVerified?.integrationLevel);
  const applied = {
    primaryType: false,
    typeName: false,
    instinctualVariant: false,
  ...
  const currentPrimaryType = normalizeTypeNumber(nextParsedData?.primaryType);
  if (resolvedPrimaryType != null
    && (currentPrimaryType == null || (config.lockOnMismatch && currentPrimaryType !== resolvedPrimaryType))) {
    nextParsedData.primaryType = resolvedPrimaryType;
    nextParsedData.primary_type = String(resolvedPrimaryType);
    nextParsedData.core_type = resolvedPrimaryType;
    nextParsedData.typeScores = buildTypeScoresFromPrimaryType(resolvedPrimaryType);
    ...
    applied.primaryType = true;
  }

  const currentInstinctualVariant = normalizeInstinctualVariant(nextParsedData?.instinctualVariant);
  if (resolvedInstinctualVariant
    && (!currentInstinctualVariant || (config.lockOnMismatch && currentInstinctualVariant !== resolvedInstinctualVariant))) {
    nextParsedData.instinctualVariant = resolvedInstinctualVariant;
    nextParsedData.dominant_instinct = resolvedInstinctualVariant;
    nextParsedData.instinctScores = buildInstinctScoresFromVariant(resolvedInstinctualVariant);
    if (nextParsedData?.attachedProfile?.core_profile?.instinctual_subtype
    ...
    applied.instinctualVariant = true;
  }

  const currentTypeName = stringOrNull(nextParsedData?.typeName);
  const shouldUpdateTypeName = Boolean(resolvedTypeName)
    && (
      !currentTypeName
      || isPlaceholderTypeName(currentTypeName)
      || applied.primaryType
    );
  ...
  const currentIntegrationLevel = normalizeIntegrationValue(nextParsedData?.integrationLevel);
  if (resolvedIntegrationLevel
    && (!currentIntegrationLevel || (config.lockOnMismatch && currentIntegrationLevel !== resolvedIntegrationLevel))) {
    nextParsedData.integrationLevel = resolvedIntegrationLevel;
    if (nextParsedData?.attachedProfile?.core_profile && typeof nextParsedData.attachedProfile.core_profile === "object") {
      nextParsedData.attachedProfile = {
        ...nextParsedData.attachedProfile,
    ...
    applied.integrationLevel = true;
  }

  console.log("[parsePdf] Identity safeguard evaluation completed.", {
    source: config.source,
    lockOnMismatch: config.lockOnMismatch,
    evidencePrimaryType,
    evidenceInstinctualVariant,
    priorPrimaryType: config?.priorVerified?.primaryType || null,
    priorInstinctualVariant: config?.priorVerified?.instinctualVariant || null,
    applied,
  });
...
function findFirstMarkerIndex(sourceLowerCase, markers) {
  const normalizedSource = String(sourceLowerCase || "");
  if (!normalizedSource) return -1;
  const markerValues = asArray(markers)
  ...
  for (const marker of markerValues) {
    ...
    if (foundIndex < 0 || markerIndex < foundIndex) {
...
function routeRepairedHtmlSegments(repairedHtml, options = {}) {
  const normalizedHtml = sanitizePdfExtractedText(repairedHtml, { preserveLineBreaks: true });
  if (!normalizedHtml) return [];

  const maxSegmentChars = Number.isFinite(Number(options?.maxSegmentChars)) && Number(options.maxSegmentChars) > 3000
    ? Math.floor(Number(options.maxSegmentChars))
    : REPAIRED_HTML_SEGMENT_MAX_CHARS;
  const overlapChars = Number.isFinite(Number(options?.overlapChars)) && Number(options.overlapChars) > 0
    ? Math.min(Math.floor(Number(options.overlapChars)), Math.floor(maxSegmentChars * 0.4))
    : REPAIRED_HTML_SEGMENT_OVERLAP_CHARS;
  const minSegmentChars = Number.isFinite(Number(options?.minSegmentChars)) && Number(options.minSegmentChars) > 0
    ? Math.floor(Number(options.minSegmentChars))
    : REPAIRED_HTML_MIN_SEGMENT_CHARS;

  const loweredHtml = normalizedHtml.toLowerCase();
  const locatedSections = REPAIRED_HTML_SECTION_BLUEPRINTS
    .map((section) => ({
      ...section,
      index: findFirstMarkerIndex(loweredHtml, section?.markers),
    }))
    .filter((section) => Number.isFinite(section.index) && section.index >= 0)
    .sort((left, right) => left.index - right.index);

  const fallbackChunks = splitRawTextIntoChunks(normalizedHtml, maxSegmentChars, {
    overlapChars,
  })
    .map((chunkText, index) => ({
      id: `fallback_segment_${index + 1}`,
      title: `Fallback Segment ${index + 1}`,
      focus: "all extractable fields",
      text: chunkText,
      start: null,
      end: null,
    }));
  ...
  if (locatedSections.length === 0) {
    return fallbackChunks.slice(0, 8);
  }

  const segmentCandidates = [];
  const firstSection = locatedSections[0];
  if (firstSection.index > minSegmentChars) {
    const introEnd = Math.min(normalizedHtml.length, Math.min(firstSection.index + overlapChars, maxSegmentChars));
    const introText = normalizedHtml.slice(0, introEnd).trim();
    if (introText.length >= minSegmentChars) {
      segmentCandidates.push({
        id: "overview",
        title: "Overview",
        focus: "client metadata and opening context",
        text: introText,
        start: 0,
        end: introEnd,
      });
  ...
  for (let sectionIndex = 0; sectionIndex < locatedSections.length; sectionIndex += 1) {
    const section = locatedSections[sectionIndex];
    const nextSectionStart = locatedSections[sectionIndex + 1]?.index ?? normalizedHtml.length;
    const start = Math.max(0, section.index - overlapChars);
    let end = Math.min(normalizedHtml.length, nextSectionStart + overlapChars);
    if (end <= start) {
      end = Math.min(normalizedHtml.length, start + maxSegmentChars);
    }
    if (end - start > maxSegmentChars) {
      end = start + maxSegmentChars;
    }
    const segmentText = normalizedHtml.slice(start, end).trim();
    if (segmentText.length < minSegmentChars && sectionIndex !== locatedSections.length - 1) continue;
  ...
  const highestCoveredEnd = segmentCandidates.reduce((maxValue, entry) => {
    const safeEnd = Number.isFinite(Number(entry?.end)) ? Number(entry.end) : 0;
    return Math.max(maxValue, safeEnd);
  }, 0);
  if (normalizedHtml.length - highestCoveredEnd > minSegmentChars) {
    const tailStart = Math.max(0, normalizedHtml.length - maxSegmentChars);
    const tailText = normalizedHtml.slice(tailStart).trim();
    if (tailText) {
      segmentCandidates.push({
        id: "tail_segment",
        title: "Tail Segment",
        focus: "remaining report sections and trailing copy",
        text: tailText,
        start: tailStart,
  ...
  for (const segment of segmentCandidates) {
    const normalizedSegmentText = stringOrNull(segment?.text);
    if (!normalizedSegmentText) continue;
    const signature = normalizeWhitespace(normalizedSegmentText.slice(0, 900));
...
function buildUserContent({ rawText, ragContext, extractionLearningContext, segment }) {
  ...
  const segmentTitle = stringOrNull(segment?.title);
  const segmentFocus = stringOrNull(segment?.focus);
  const segmentInstruction = segmentTitle
    ? `You are parsing a targeted HTML segment: "${segmentTitle}". Prioritize semantic mapping for: ${segmentFocus || "all matching schema fields"} and leave unsupported fields null/empty.`
    : "You are parsing the full repaired HTML report. Use semantic mapping and leave unsupported fields null/empty.";
  const markdownInstruction = ATTACHED_JSON_SEMANTIC_EXTRACTION_GUIDANCE;
  if (combinedPrefix) {
...
function mergeStructuredPair(baseValue, incomingValue, pathParts = []) {
  if (incomingValue == null) return baseValue;
  if (baseValue == null) return incomingValue;
  const pathKey = Array.isArray(pathParts) ? pathParts.join(".") : "";
  const shouldUseLateWinIdentityMerge = SEGMENT_MERGE_LATE_WIN_PATHS.has(pathKey);
  ...
  if (typeof baseValue === "object" && typeof incomingValue === "object") {
    ...
    for (const key of keys) {
      merged[key] = mergeStructuredPair(baseValue[key], incomingValue[key], [...pathParts, key]);
    }
    return merged;
  }
  ...
  if (typeof baseValue === "string" || typeof incomingValue === "string") {
    ...
    if (!baseText && !incomingText) return baseValue;
    if (shouldUseLateWinIdentityMerge) {
      return incomingValue;
    }
    return incomingText.length > baseText.length ? incomingValue : baseValue;
  }

  if (shouldUseLateWinIdentityMerge && (typeof incomingValue === "number" || typeof incomingValue === "string")) {
    return incomingValue;
  }
...
async function agenticOcrRepair(rawHtml, options = {}) {
  ...
  const normalizedRepairedText = normalizeOcrArtifactString(repairedText, { preserveLineBreaks: true });

  console.log("[parsePdf] Agentic OCR repair stage completed.", {
    repairedChars: normalizedRepairedText.length,
    normalizedChanged: normalizedRepairedText !== repairedText,
    sourceFileName: stringOrNull(options?.sourceFileName) || "report.pdf",
  });
...
async function extractAttachedStructuredJson({
  ...
  return normalizeOcrArtifactPayload(JSON.parse(content));
}

async function extractAttachedStructuredJsonWithTargetedRouting({
  openAiUrl,
  apiKey,
  repairedHtml,
  ragContext,
  ...
  const routedSegments = routeRepairedHtmlSegments(normalizedRepairedHtml);
  const shouldUseSegmentRouting = Boolean(forceSegmentRouting) || normalizedRepairedHtml.length > REPAIRED_HTML_SINGLE_PASS_MAX_CHARS;
  if (!shouldUseSegmentRouting || routedSegments.length <= 1) {
    return extractAttachedStructuredJson({
      openAiUrl,
      apiKey,
      repairedHtml: normalizedRepairedHtml,
      ragContext,
      extractionLearningContext,
      requestFn,
      segment: null,
  ...
  console.log("[parsePdf] Repaired HTML exceeded single-pass target; routing targeted extraction segments.", {
    repairedChars: normalizedRepairedHtml.length,
    segmentCount: routedSegments.length,
    singlePassThreshold: REPAIRED_HTML_SINGLE_PASS_MAX_CHARS,
  });
  ...
  for (let index = 0; index < routedSegments.length; index += 1) {
    ...
    try {
      const partialStructured = await extractAttachedStructuredJson({
        openAiUrl,
        apiKey,
        repairedHtml: segment.text,
        ragContext,
        extractionLearningContext,
        requestFn,
        segment,
      ...
      if (partialStructured && typeof partialStructured === "object") {
        structuredSegments.push(partialStructured);
      }
      console.log("[parsePdf] Targeted segment parsed.", {
        segment: `${index + 1}/${routedSegments.length}`,
      ...
      console.log("[parsePdf] Targeted segment parse failed.", {
        segment: `${index + 1}/${routedSegments.length}`,
        segmentId: segment.id,
        segmentTitle: segment.title,
        chars: segment.text.length,
  ...
  if (structuredSegments.length === 0) {
    console.log("[parsePdf] All targeted segment parses failed; retrying single-pass extraction.", {
      failedSegments: failedSegments.map((entry) => entry.id),
    });
    return extractAttachedStructuredJson({
      openAiUrl,
      apiKey,
  ...
  const mergedStructured = mergeStructuredObjects(structuredSegments);
  if (!mergedStructured || typeof mergedStructured !== "object") {
    throw new Error("Targeted segment extraction did not produce a valid structured payload.");
  }

  if (failedSegments.length > 0) {
    console.log("[parsePdf] Targeted segment extraction completed with partial failures.", {
      successfulSegments: structuredSegments.length,
      failedSegments: failedSegments.map((entry) => entry.id),
    });
  }
...
async function extractStructuredJsonFromMarkdown({
  ...
  return extractAttachedStructuredJsonWithTargetedRouting({
    openAiUrl,
    apiKey,
...
function mapAttachedToLegacyPayload({
  ...
  const centerScores = {
    body: levelLabelToVisualScore(actionLabel),
    heart: levelLabelToVisualScore(feelingLabel),
    head: levelLabelToVisualScore(thinkingLabel),
  };
  ...
  const splitNarrativeRows = (value, { maxItems = 8 } = {}) => {
    ...
    if (symbolSplit.length > 1) {
      return Array.from(new Set(symbolSplit)).slice(0, Math.max(1, Number(maxItems) || 8));
    }
...
export async function parsePdf(pdfBuffer, optionsOrId) {
  ...
  const identitySafeguardConfig = normalizeIdentitySafeguardConfig(parseOptions?.identitySafeguard);
  ...
  let lastKnownDetectedTotalPages = null;
  ...
  try {
    const endpointRaw = normalizeAzureEnvVar(process.env.AZURE_OPENAI_ENDPOINT, { stripTrailingSlash: true });
    // Normalize portal-provided endpoints that may include '/openai/...'
    const { normalizeAzureOpenAiEndpoint } = await import("./normalizeAzureOpenAiEndpoint.js");
    ...
    const apiVersion = normalizeAzureEnvVar(process.env.AZURE_OPENAI_API_VERSION) || "2025-04-01-preview";
    const openAiUrl = `${endpoint}/openai/deployments/${deployment}/chat/completions?api-version=${encodeURIComponent(apiVersion)}`;
    ...
    if (rawTextOverride) {
      ...
      lastKnownDetectedTotalPages =
        pageCountOverride ||
        (hasPagesOverrideText ? extractedPages.length : null) ||
        null;
      ...
      lastKnownDetectedTotalPages = detectedPageCount || null;
      ...
      extractedPages = layoutHtmlOverride
        ? buildOverridePages(layoutHtmlOverride, effectivePageCount)
        : await extractPdfPagesWithPython(pdfBuffer);
      lastKnownExtractedPageCount = extractedPages.length;
      const extractedRawText = buildRawTextFromPages(extractedPages, { withPageMarkers: true });
      if (!extractedRawText) {
        throw new Error("No extractable PDF text found.");
      }
      ...
      if (enableCanonicalRag) {
        ...
        lastKnownCanonicalRag = canonicalRagContext;
        console.log(
          "[parsePdf] Canonical RAG context refreshed from extracted PDF text.",
          summarizeCanonicalRagDiagnostics(canonicalRagContext),
        );
      }

      extractionMethod = "local_text_only";
      console.log("[parsePdf] Using text-only PDF extraction for LLM parsing.", {
        pages: extractedPages.length,
        chars: extractedRawText.length,
        sourceFileName,
        parserVersion: PARSER_VERSION,
      });
      structured = await extractStructuredJsonFromRawText({
    ...
    const deterministicFallbackApplied = {
      primaryType:
        ...
        identityAfterDeterministicFallback.primaryType !== identityBeforeDeterministicFallback.primaryType,
      instinctualVariant:
        Boolean(identityAfterDeterministicFallback.instinctualVariant) &&
        identityAfterDeterministicFallback.instinctualVariant !== identityBeforeDeterministicFallback.instinctualVariant,
    };

    const identitySafeguardResult = applyIdentitySafeguardToParsedData({
      parsedData,
      rawText: deterministicFallbackRawText,
      identitySafeguardConfig,
    });
    parsedData = identitySafeguardResult.parsedData;
    ...
    verification = {
      ...
      fallbackApplied: {
        ...
        instinctualVariant:
          ...
          Boolean(identitySafeguardResult?.applied?.instinctualVariant),
        typeName:
          Boolean(fallbackResult?.fallbackApplied?.typeName) ||
          Boolean(identitySafeguardResult?.applied?.typeName),
        integrationLevel:
          Boolean(fallbackResult?.fallbackApplied?.integrationLevel) ||
          Boolean(identitySafeguardResult?.applied?.integrationLevel),
      },
      resolvedFields: {
        ...verification.resolvedFields,
        primaryType:
          identitySafeguardResult?.resolved?.primaryType ||
          verification?.resolvedFields?.primaryType ||
          normalizeTypeNumber(parsedData?.primaryType) ||
          inferTypeNumberFromTypeName(parsedData?.typeName) ||
          null,
        typeName:
          stringOrNull(parsedData?.typeName) ||
          identitySafeguardResult?.resolved?.typeName ||
          verification?.resolvedFields?.typeName ||
          null,
        instinctualVariant:
          identitySafeguardResult?.resolved?.instinctualVariant ||
          verification?.resolvedFields?.instinctualVariant ||
          normalizeInstinctualVariant(parsedData?.instinctualVariant) ||
          null,
        integrationLevel:
          identitySafeguardResult?.resolved?.integrationLevel ||
    ...
    const verifiedDetectedTotalPages = Number.isFinite(Number(verification?.python?.pageCount))
      ...
      : (Number.isFinite(Number(lastKnownDetectedTotalPages)) && Number(lastKnownDetectedTotalPages) > 0
          ? Math.floor(Number(lastKnownDetectedTotalPages))
          : null);
    ...
    const hasCoverageComplete = Boolean(parseCoverage?.isCoverageComplete);
    const isComplete = hasCoverageComplete
      && hasCoreProfile
      && (!requireChartScoresForComplete || hasAllChartScores);

    const incompleteReason = !hasCoverageComplete
      ? (parseCoverage?.detectedTotalPages != null
          ? `Extracted ${extractedPageCount} pages, detected ${parseCoverage.detectedTotalPages}`
    ...
    if (parseCoverage?.detectedTotalPages != null
      && parseCoverage?.minExpectedPages != null
      && parseCoverage.detectedTotalPages < parseCoverage.minExpectedPages) {
      parserWarnings.push({
        message: "Detected total pages below configured minimum threshold",
        details: `Detected ${parseCoverage.detectedTotalPages} pages; configured minimum is ${parseCoverage.minExpectedPages}. Completeness uses detected page coverage.`,
      });
    }
    ...
    return {
      ...
      _parseDiagnostics: {
        ...
        identitySafeguard: {
          enabled: Boolean(identitySafeguardConfig?.enabled),
          source: identitySafeguardConfig?.source || null,
          lockOnMismatch: Boolean(identitySafeguardConfig?.lockOnMismatch),
          priorVerified: identitySafeguardConfig?.priorVerified || null,
        },
    ...
    const parseCoverage = buildParseCoverage({
      ...
      detectedTotalPages: lastKnownDetectedTotalPages,
      minExpectedPages: expectedPages,
    });
...
export {
  ...
  extractJsonPayloadFromPythonStdout,
  extractAttachedStructuredJson,
  extractAttachedStructuredJsonWithTargetedRouting,
