...
I will provide structurally-aware markdown text extracted from an iEQ9 Individual Professional Enneagram Report.

Your task is to parse the markdown report content and output a structured JSON object.
Extract the required JSON strictly from the following markdown report.
...
function buildUserContent({ rawText, ragContext, extractionLearningContext }) {
  ...
  if (!normalizedRawText) {
    throw new Error("Structured markdown text is required for LLM parsing.");
  }
  const markdownInstruction = "Extract the required JSON strictly from the following markdown report.";
  if (combinedPrefix) {
...
function mapAttachedToLegacyPayload({
  ...
  return {
    ...
    reportSummary: `Markdown-first extraction completed for ${reportId || "uploaded report"}.`,
    reportContent: {
      documentSummary: `Processed ${pageSnapshots.length} pages using Azure Document Intelligence markdown + LLM parser.`,
...
export async function parsePdf(pdfBuffer, optionsOrId) {
  ...
  let preloadedPythonVerification = null;
  let preflightMissingEnvVars = [];
  let preflightMissingDocIntelEnvVars = [];
  let docIntelRequiredForRun = false;

  console.log(`[parsePdf] Starting markdown-first parsing for ${reportId || "new report"}...`);
  ...
  try {
    ...
    docIntelRequiredForRun = !rawTextOverride;
    ...
    preflightMissingEnvVars = missingByProvider.openAi;
    ...
    if (rawTextOverride) {
      ...
      console.log("[parsePdf] Using provided rawTextOverride for markdown-first LLM parse.", {
      ...
      try {
        preloadedPythonVerification = await extractDashboardVerificationWithPython({
          pdfBuffer,
          sourceFileName,
          verificationOverride: pythonVerificationOverride,
        });
      } catch (pythonVerificationError) {
        preloadedPythonVerification = {
          available: false,
          source: "python_extract_report_pdf",
          reason: "python_verification_failed",
          details: String(pythonVerificationError?.message || pythonVerificationError),
          markdown: null,
        };
      }
      const doclingMarkdown = stringOrNull(
        sanitizePdfExtractedText(preloadedPythonVerification?.markdown, { preserveLineBreaks: true }),
      );
      if (doclingMarkdown) {
        extractedPages = buildOverridePages(doclingMarkdown, effectivePageCount);
        lastKnownExtractedPageCount = extractedPages.length;
        extractionMethod = "python_docling_markdown";
        console.log("[parsePdf] Using python Docling markdown pre-processor output for LLM parsing.", {
          pages: extractedPages.length,
          chars: doclingMarkdown.length,
          sourceFileName,
          parserVersion: PARSER_VERSION,
        });
        if (enableCanonicalRag) {
          canonicalRagContext = await buildCanonicalRagContext({
            enabled: true,
            rawText: doclingMarkdown,
            sourceFileName,
          });
          lastKnownCanonicalRag = canonicalRagContext;
          console.log(
            "[parsePdf] Canonical RAG context refreshed from python Docling markdown.",
        ...
        console.log("[parsePdf] Extracting markdown with Azure Document Intelligence pre-processor.", {
        ...
        try {
          const extractedMarkdown = await extractMarkdownWithAzureDocIntel(pdfBuffer, {
            endpoint: docIntelEndpoint,
            apiKey: docIntelApiKey,
            sourceFileName,
          });
          extractedPages = buildOverridePages(extractedMarkdown, effectivePageCount);
          lastKnownExtractedPageCount = extractedPages.length;
          ...
          if (enableCanonicalRag) {
            ...
            lastKnownCanonicalRag = canonicalRagContext;
            console.log(
              "[parsePdf] Canonical RAG context refreshed from Azure Document Intelligence markdown.",
              summarizeCanonicalRagDiagnostics(canonicalRagContext),
            );
          }

          extractionMethod = "azure_doc_intelligence_markdown";
          structured = await extractStructuredJsonFromRawText({
            openAiUrl,
            apiKey,
            rawText: extractedMarkdown,
          ...
          if (!allowLocalTextFallback) throw docIntelError;

          console.log("[parsePdf] Azure Document Intelligence extraction failed; retrying through local full-text extraction fallback.", {
            details: String(docIntelError?.message || docIntelError),
            parserVersion: PARSER_VERSION,
          });
    ...
    const pythonVerification = enablePythonCrossCheck
      ? (
        preloadedPythonVerification && typeof preloadedPythonVerification === "object"
          ? preloadedPythonVerification
    ...
    return {
      ...
      _parseDiagnostics: {
        ...
        preflight: {
          ...
          hasAzureDocIntelConfig: docIntelRequiredForRun ? preflightMissingDocIntelEnvVars.length === 0 : true,
        },
      },
    };
  } catch (error) {
    console.error("[parsePdf] Fatal error during markdown-first parsing:", error);
