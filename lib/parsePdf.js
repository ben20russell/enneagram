...
5. PUNCTUATION ENFORCEMENT: If original bullet points or list items lack periods, you MUST add a period at the end of each complete thought. If extracting a list into a single string field, separate distinct items with periods. Never merge separate bullet points into a run-on sentence without punctuation.
6. Do not infer from image layout; use only provided repaired text.
7. Follow this targeted extraction guidance for higher-accuracy section capture:
...
8. Output ONLY valid JSON matching this structure:
{
