...
function levelToVisualScore(level) {
...
function extractScoresFromText(text) {
  ...
  const actionQualitativeLevel = normalizeLevelLabel(actionQualitative?.[1]);
  const feelingQualitativeLevel = normalizeLevelLabel(feelingQualitative?.[1]);
  const thinkingQualitativeLevel = normalizeLevelLabel(thinkingQualitative?.[1]);
  if (actionQualitativeLevel) centerScores.body = actionQualitativeLevel;
  if (feelingQualitativeLevel) centerScores.heart = feelingQualitativeLevel;
  if (thinkingQualitativeLevel) centerScores.head = thinkingQualitativeLevel;
...
export async function parsePdf(pdfBuffer, optionsOrId) {
  ...
  try {
    ...
    if (!parsedData.centerScores.body && !parsedData.centerScores.heart && !parsedData.centerScores.head) {
      parsedData.centerScores = { body: "Low", heart: "Low", head: "Low" };
    }
