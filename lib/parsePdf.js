...
const ieq9TargetedExtractionConfig = JSON.parse(
  readFileSync(new URL("../config/ieq9_targeted_extraction_config.json", import.meta.url), "utf8"),
);

const TARGETED_SECTION_PAGE_MAP =
  ieq9TargetedExtractionConfig?.section_page_map &&
  typeof ieq9TargetedExtractionConfig.section_page_map === "object"
    ? ieq9TargetedExtractionConfig.section_page_map
    : {};

const TARGETED_DEVELOPMENT_CONTEXT_PAGE_MAP =
  ieq9TargetedExtractionConfig?.development_exercise_context_pages &&
  typeof ieq9TargetedExtractionConfig.development_exercise_context_pages === "object"
    ? ieq9TargetedExtractionConfig.development_exercise_context_pages
    : {};

const TARGETED_SECTION_HEADER_TITLES =
  ieq9TargetedExtractionConfig?.section_header_titles &&
  typeof ieq9TargetedExtractionConfig.section_header_titles === "object"
    ? ieq9TargetedExtractionConfig.section_header_titles
    : {};

const TARGETED_FOOTER_PATTERN = new RegExp(
  String(
    ieq9TargetedExtractionConfig?.footer_pattern ||
      "Copyright\\s+\\d{4}-\\d{4}\\s+Integrative\\s+Enneagram\\s+Solutions[\\s\\S]*?\\b\\d+\\s+of\\s+42\\b",
  ),
  "gi",
);

const ieq9_targeted_sections_schema =
  ieq9TargetedExtractionConfig?.output_schema &&
  typeof ieq9TargetedExtractionConfig.output_schema === "object"
    ? ieq9TargetedExtractionConfig.output_schema
...
4. Extract all relevant PRO sections (Team Dynamics, Strategic Leadership, etc.) and include them in the reportContent.proSections object.`,
    ...
    if (targetedSectionEntries.length) {
      parsedData.targetedSections = targetedStructuredData;
