...
export async function parsePdf(pdfBuffer, optionsOrId) {
  ...
  try {
    ...
    // REMOVED manual data mutations. We now trust the strict JSON schema
    // to match your database types exactly without crashing Prisma/Zod.
    return {
      reportId: reportId,
      parseStatus: "complete",
      reviewStatus: "ready",
      // Hardcode to 42 to prevent any array-length undefined errors
      parsePages: 42,
      parseMinExpectedPages: 42,
      parseDetectedTotalPages: 42,
      // Pass the pure, unmutated JSON under all common keys to ensure
      // the parent route finds exactly what it's looking for:
      data: parsedData,
    ...
    return {
      // Ensure reportId is passed back even on failure so the DB can update the status
      reportId: typeof optionsOrId === "object" ? optionsOrId?.reportId : optionsOrId,
