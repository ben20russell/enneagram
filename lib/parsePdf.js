...
const CANONICAL_RAG_SOURCE_URL = new URL("../docs/enneagram-master-source.txt", import.meta.url);
const CANONICAL_RAG_SOURCE_PATH = fileURLToPath(CANONICAL_RAG_SOURCE_URL);
...
const CANONICAL_RAG_LEGAL_NOISE_PATTERN =
  /\b(copyright|all rights reserved|library of congress|printed in the united states|bantam books)\b/i;
...
let canonicalRagChunksCache = null;
...
function splitCanonicalReferenceIntoChunks(sourceText, chunkTargetChars = CANONICAL_RAG_CHUNK_TARGET_CHARS) {
  ...
  const paragraphs = String(sourceText || "")
    ...
    .filter(Boolean)
    .filter((entry) => !CANONICAL_RAG_LEGAL_NOISE_PATTERN.test(entry));
...
async function loadCanonicalRagChunks() {
  if (canonicalRagChunksCache) {
    return canonicalRagChunksCache;
  }

  try {
    const sourceText = await fs.readFile(CANONICAL_RAG_SOURCE_PATH, "utf8");
    const chunks = splitCanonicalReferenceIntoChunks(sourceText, CANONICAL_RAG_CHUNK_TARGET_CHARS);
    canonicalRagChunksCache = {
      sourcePath: CANONICAL_RAG_SOURCE_PATH,
      chunkCount: chunks.length,
      chunks,
    };
    console.log("[parsePdf] Canonical RAG source loaded.", {
      sourcePath: CANONICAL_RAG_SOURCE_PATH,
      chunkCount: chunks.length,
    });
    return canonicalRagChunksCache;
  } catch (error) {
    console.log("[parsePdf] Canonical RAG source load failed.", {
      sourcePath: CANONICAL_RAG_SOURCE_PATH,
...
function buildCanonicalRagQuery({ rawText, sourceFileName }) {
...
function scoreCanonicalChunk(queryTokenSet, chunkTokenSet) {
  if (!queryTokenSet || !chunkTokenSet || queryTokenSet.size === 0 || chunkTokenSet.size === 0) return 0;
  let overlap = 0;
  ...
  const density = overlap / Math.sqrt(chunkTokenSet.size);
...
function selectCanonicalRagChunks({
  ...
  const queryTokenSet = new Set(asArray(queryTokens));

  const scored = normalizedChunks
    .map((chunk) => ({
      ...chunk,
      score: scoreCanonicalChunk(queryTokenSet, chunk?.tokenSet),
    }))
  ...
  if (selected.length === 0 && scored.length > 0) {
    const fallbackChunk = scored[0];
    if (fallbackChunk?.text) {
      selected.push(fallbackChunk);
    }
  }
...
async function buildCanonicalRagContext({
  enabled,
  rawText,
  sourceFileName,
  maxChunks = CANONICAL_RAG_MAX_CHUNKS,
  ...
  const source = await loadCanonicalRagChunks();
  ...
  const query = buildCanonicalRagQuery({ rawText, sourceFileName });
  ...
  const content = selectedChunks
    .map((chunk, index) => `[Reference ${index + 1}] ${chunk.text}`)
    .join("\n\n");

  console.log("[parsePdf] Canonical RAG context retrieved.", {
    sourcePath: source.sourcePath,
    queryTokenCount: queryTokens.length,
    retrievedChunkCount: selectedChunks.length,
...
function buildCanonicalRagPromptPrefix(ragContext) {
  ...
  return [
    "Retrieved canonical Enneagram reference excerpts:",
    ragText,
    "",
    "Use these excerpts only for terminology normalization and canonical wording.",
...
function summarizeCanonicalRagDiagnostics(ragContext) {
  return {
    ...
    sourcePath: stringOrNull(ragContext?.sourcePath) || CANONICAL_RAG_SOURCE_PATH,
...
function normalizePythonVerificationPayload(payload, sourceFileName) {
  ...
  const detectedType = normalizeTypeNumber(payload?.detectedType);
  ...
  return {
    ...
    detectedTypeSource: stringOrNull(payload?.detectedTypeSource),
    typeName: stringOrNull(payload?.typeName),
    instinctCode,
    instinctLabel,
    integrationLevel,
    containsMarkers:
...
function buildPythonVerificationCrossCheck({
  ...
  const resolvedFields = {
    ...
    primaryType: pythonPrimaryType || normalizeTypeNumber(parsedData?.primaryType),
    // Keep LLM as preferred source for narrative identity naming.
    typeName: stringOrNull(parsedData?.typeName) || pythonTypeName,
...
function applyPythonVerificationFallbacksToParsedData(parsedData, verification) {
  ...
  if (!stringOrNull(nextParsedData?.typeName) && resolvedTypeName) {
...
function buildUserContent({ rawText, pdfBuffer, sourceFileName, ragContext }) {
...
function splitRawTextIntoChunks(rawText, maxChunkChars) {
...
function mergeStructuredObjects(baseValue, incomingValue) {
  ...
  if (typeof baseValue === "object" && typeof incomingValue === "object") {
    ...
    for (const key of keys) {
      merged[key] = mergeStructuredObjects(baseValue[key], incomingValue[key]);
    }
    return merged;
  }
...
async function extractStructuredJsonFromRawText({
  openAiUrl,
  apiKey,
  rawText,
  sourceFileName,
  ...
  for (let index = 0; index < chunks.length; index += 1) {
    ...
    try {
      const partialStructured = await extractAttachedStructuredJson({
        openAiUrl,
        apiKey,
        rawText: `This is chunk ${index + 1} of ${chunks.length} from the full report "${sourceFileName || "report.pdf"}".\nExtract whatever fields are present in this chunk and leave unknown fields empty.\n\n${chunkText}`,
        pdfBuffer: null,
        sourceFileName,
        ragContext,
      });
      mergedStructured = mergeStructuredObjects(mergedStructured, partialStructured);
      successfulChunks += 1;
...
function mapAttachedToLegacyPayload({
  ...
  const primaryType = normalizeTypeNumber(core?.type_number);
...
export async function parsePdf(pdfBuffer, optionsOrId) {
  ...
  try {
    ...
    verification = {
      ...
      resolvedFields: {
        ...verification.resolvedFields,
        primaryType: verification?.resolvedFields?.primaryType || normalizeTypeNumber(parsedData?.primaryType) || null,
