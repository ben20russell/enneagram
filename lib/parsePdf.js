...
export async function parsePdf(pdfBuffer, optionsOrId) {
  ...
  try {
    ...
    // -----------------------------------------------------------------
    // STEP F: Final Clean & Return
    // -----------------------------------------------------------------
    console.log(`[parsePdf] Success! Returning complete payload.`);

    // 1. Build the strict return object with NO extra keys
    const finalPayload = {
      parseStatus: "complete",
      reviewStatus: "ready",
      parsePages: pages?.length || 42,
      parseMinExpectedPages: 42,
      parseDetectedTotalPages: pages?.length || 42,
      results_data: parsedData,
    };

    // 2. Only attach the reportId if it actually exists to prevent 'undefined' DB crashes
    if (typeof optionsOrId === "string") {
      finalPayload.reportId = optionsOrId;
    } else if (optionsOrId && optionsOrId.reportId) {
      finalPayload.reportId = optionsOrId.reportId;
    }
    ...
    if (typeof optionsOrId === "string") {
      errorPayload.reportId = optionsOrId;
    } else if (optionsOrId && optionsOrId.reportId) {
      errorPayload.reportId = optionsOrId.reportId;
    }
