...
function buildPythonVerificationCrossCheck({
  ...
  const resolvedFields = {
    primaryType: normalizeTypeNumber(parsedData?.primaryType) || pythonPrimaryType,
    typeName: stringOrNull(parsedData?.typeName) || pythonTypeName,
    instinctualVariant: normalizeInstinctualVariant(parsedData?.instinctualVariant) || pythonInstinctualVariant,
    integrationLevel: normalizeIntegrationValue(parsedData?.integrationLevel) || pythonIntegrationLevel,
...
export async function parsePdf(pdfBuffer, optionsOrId) {
  ...
  try {
    ...
    verification = {
      ...
      resolvedFields: {
        ...verification.resolvedFields,
        primaryType: normalizeTypeNumber(parsedData?.primaryType) || verification?.resolvedFields?.primaryType || null,
