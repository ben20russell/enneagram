...
async function importPdfJsLegacyBuild() {
  ...
  const specifiers = ["pdfjs-dist/legacy/build/pdf.mjs", "pdfjs-dist/legacy/build/pdf.js"];
  let lastError = null;
  for (const specifier of specifiers) {
    try {
      return await dynamicImportRuntime(specifier);
    } catch (error) {
      lastError = error;
    }
  }
...
export async function parsePdf(pdfBuffer) {
  ...
  const configuredMinExpectedPages = Math.max(1, Math.floor(Number(process.env.PDF_PARSE_MIN_PAGES || 20)));
