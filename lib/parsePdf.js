...
// Equivalent legacy CommonJS path: require("pdfjs-dist/legacy/build/pdf.js")
function getLegacyPdfJsCjsPath() {
  return ["pdfjs-dist", "legacy", "build", "pdf.js"].join("/");
}
...
async function importPdfJsLegacyBuild() {
  let lastError = null;
  try {
    // Keep CommonJS legacy path first for older/runtime-constrained Node environments.
    return normalizePdfJsModule(nodeRequire(getLegacyPdfJsCjsPath()));
  } catch (error) {
    lastError = error;
  }

  const dynamicImportRuntime = new Function("specifier", "return import(specifier)");
  const specifiers = ["pdfjs-dist/legacy/build/pdf.js", "pdfjs-dist/legacy/build/pdf.mjs"];
  for (const specifier of specifiers) {
    try {
      return normalizePdfJsModule(await dynamicImportRuntime(specifier));
    } catch (error) {
      lastError = error;
    }
  }
