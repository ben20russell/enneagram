...
export async function parsePdf(input, options = {}) {
  ...
  const parseStatus = hasMinimumPages && hasCoreResults ? "complete" : "incomplete";
  ...
  } else if (parseStatus === "incomplete" && !hasCoreResults) {
    diagnostics.isComplete = false;
    diagnostics.incompleteReason =
      "Core results incomplete: missing one or more required top-level fields (core_type, instinct).";
    pushError("parse_incomplete_missing_core_results", diagnostics.incompleteReason);
  } else {
