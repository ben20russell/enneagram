...
const PRIMARY_PARSER_VERSION = "single-pass-primary-v1";
const PRIMARY_VALIDATION_RETRY_LIMIT = 1;
const PRIMARY_CRITICAL_STRAIN_KEYS = [
  "happiness",
  "vocational",
  "interpersonal",
  "physical",
...
const PRIMARY_CRITICAL_CENTER_KEYS = ["action", "feeling", "thinking"];
const CENTER_EXPRESSION_PAGE_RANGE = { start: 12, end: 13 };
const CENTER_EXPRESSION_HEADING_LABELS = [
  "Action Center of Expression",
  "Feeling Center of Expression",
  "Thinking Center of Expression",
];
const TARGETED_STRAIN_INTERPRETATION_RULES = {
  environmental: {
    pageNumbers: [20],
...
function normalizeTargetedAnchor(anchor) {
...
function findTargetedAnchorIndex(source, anchor, startIndex = 0) {
  const haystack = String(source || "");
  const needle = String(anchor || "").trim();
  if (!haystack || !needle) return -1;
  return haystack.toLowerCase().indexOf(needle.toLowerCase(), Math.max(0, Number(startIndex) || 0));
}

function extractTargetedAnchoredSnippet(source, rule) {
  const text = String(source || "").trim();
  if (!text) return "";
  const safeRule = rule && typeof rule === "object" ? rule : {};
  const startAnchor = normalizeTargetedAnchor(safeRule.startAnchor);
  const endAnchor = normalizeTargetedAnchor(safeRule.endAnchor);
  let startIndex = 0;
  if (startAnchor && startAnchor !== "end_of_page") {
    const index = findTargetedAnchorIndex(text, startAnchor, 0);
    if (index === -1) return "";
    startIndex = index;
  }
  let endIndex = text.length;
  if (endAnchor && endAnchor !== "end_of_page") {
    const index = findTargetedAnchorIndex(text, endAnchor, startIndex + 1);
    if (index !== -1) {
      endIndex = index;
    }
  }
...
function buildTargetedStrainInterpretationTextByName(pageTextByNumber) {
  ...
  const chunks = [];
  for (const [key, rule] of Object.entries(TARGETED_STRAIN_INTERPRETATION_RULES)) {
    const pageNumbers = Array.isArray(rule?.pageNumbers) ? rule.pageNumbers : [];
    const source = pageNumbers
      .map((pageNumber) => String(pageTextByNumber?.[pageNumber] || "").trim())
      .filter(Boolean)
      .map((text) => stripTargetedFooterNoise(text))
      .filter(Boolean)
      .join("\n\n")
      .trim();
    if (!source) continue;
    const snippet = extractTargetedAnchoredSnippet(source, rule);
    if (!snippet) continue;
    chunks.push(`[${categoryLabels[key] || key}] ${snippet}`);
  }
  return chunks.join("\n\n").trim();
}
...
function buildTargetedSectionTextByName(pageTextByNumber) {
  ...
  for (const [sectionName, pageNumbers] of Object.entries(TARGETED_SECTION_PAGE_MAP)) {
    if (sectionName === "strain_interpretation") {
      const strainTargetedText = buildTargetedStrainInterpretationTextByName(pageTextByNumber);
      if (strainTargetedText) {
        cleaned[sectionName] = strainTargetedText;
        continue;
      }
    }
    const chunks = [];
...
async function extractPrimaryStructuredWithAzureOpenAi({
  openAiUrl,
  apiKey,
  masterDocumentText,
  retryContext = null,
}) {
  const baseSystemPrompt = `You are an expert Enneagram extractor. Parse the report text into JSON.
...
4. Extract all relevant PRO sections (Team Dynamics, Strategic Leadership, etc.) and include them in the reportContent.proSections object.`;
  const focusedRetryPrompt = retryContext
    ? `\n\nPRIMARY RETRY FOCUS:\n${retryContext}\nReturn JSON only and keep unchanged values if already accurate.`
    : "";
  ...
  const response = await fetch(openAiUrl, {
    ...
    body: JSON.stringify({
      messages: [
        {
          role: "system",
          content: `${baseSystemPrompt}${focusedRetryPrompt}`,
        },
  ...
  if (!response.ok) {
    throw new Error(`Azure OpenAI primary extraction error: ${await response.text()}`);
  }
...
function normalizeInstinctualVariant(value) {
  ...
  if (normalized === "sx" || normalized === "sexual") return "sx";
  if (normalized === "so" || normalized === "social") return "so";
  if (normalized === "sp" || normalized === "self-preservation" || normalized === "selfpreservation") return "sp";
  return null;
}

function inferLevelFromNarrative(value) {
  const normalized = String(value || "").trim().toLowerCase();
  if (!normalized) return null;
  if (/\bhigh\b/.test(normalized)) return "HIGH";
  if (/\bmedium\b/.test(normalized) || /\bmoderate\b/.test(normalized)) return "MEDIUM";
  if (/\blow\b/.test(normalized)) return "LOW";
  return null;
}

function asUpperLevel(value) {
  const normalized = normalizeLevelLabel(value);
  return normalized ? normalized.toUpperCase() : null;
}

function buildCriticalSnapshot(payload) {
  const source = payload && typeof payload === "object" ? payload : {};
  const centerSource =
    source?.centerLabels && typeof source.centerLabels === "object"
      ? source.centerLabels
      : source?.centers_of_expression && typeof source.centers_of_expression === "object"
        ? source.centers_of_expression
        : source?.centersOfExpression && typeof source.centersOfExpression === "object"
          ? source.centersOfExpression
          : {};
  const centers = {
    action: asUpperLevel(centerSource?.action),
    feeling: asUpperLevel(centerSource?.feeling),
    thinking: asUpperLevel(centerSource?.thinking),
  };
  const strainSource =
    source?.strainInterpretations && typeof source.strainInterpretations === "object"
      ? source.strainInterpretations
      : source?.strainNarratives && typeof source.strainNarratives === "object"
        ? source.strainNarratives
        : source?.strain_interpretations && typeof source.strain_interpretations === "object"
          ? source.strain_interpretations
          : {};
  const strainNarratives = {};
  const strainLevels = {};
  const missingStrainCategories = [];
  for (const key of PRIMARY_CRITICAL_STRAIN_KEYS) {
    const narrative = normalizeWhitespace(strainSource?.[key] || "");
    strainNarratives[key] = narrative;
    const inferredLevel = inferLevelFromNarrative(narrative);
    strainLevels[key] = inferredLevel;
    if (!narrative) missingStrainCategories.push(key);
  }
  const missingCritical = [];
  const primaryType = Number.isFinite(Number(source?.primaryType)) ? Number(source.primaryType) : null;
  const instinctualVariant = normalizeInstinctualVariant(source?.instinctualVariant);
  if (!primaryType) missingCritical.push("primary_type");
  if (!instinctualVariant) missingCritical.push("instinctual_variant");
  for (const key of PRIMARY_CRITICAL_CENTER_KEYS) {
    if (!centers[key]) missingCritical.push(`center_${key}`);
  }
  if (missingStrainCategories.length) missingCritical.push("strain_sections");
  return {
    primaryType,
    instinctualVariant,
    centers,
    ...
    strainLevels,
    missingCritical,
    missingStrainCategories,
  };
}

function computeCriticalDiff(primarySnapshot, validatorSnapshot) {
  const primary = primarySnapshot && typeof primarySnapshot === "object" ? primarySnapshot : {};
  const validator = validatorSnapshot && typeof validatorSnapshot === "object" ? validatorSnapshot : {};
  const mismatches = [];
  let comparedFields = 0;
  let centerMismatchCount = 0;
  let strainMismatchCount = 0;

  if (Number.isFinite(Number(primary?.primaryType)) && Number.isFinite(Number(validator?.primaryType))) {
    comparedFields += 1;
    if (Number(primary.primaryType) !== Number(validator.primaryType)) {
      mismatches.push("primary_type");
    }
  }
  if (primary?.instinctualVariant && validator?.instinctualVariant) {
    comparedFields += 1;
    if (primary.instinctualVariant !== validator.instinctualVariant) {
      mismatches.push("instinctual_variant");
    }
  }
  for (const key of PRIMARY_CRITICAL_CENTER_KEYS) {
    const primaryValue = String(primary?.centers?.[key] || "");
    const validatorValue = String(validator?.centers?.[key] || "");
    if (!primaryValue || !validatorValue) continue;
    comparedFields += 1;
    if (primaryValue !== validatorValue) {
      mismatches.push(`center_${key}`);
      centerMismatchCount += 1;
    }
  }
  for (const key of PRIMARY_CRITICAL_STRAIN_KEYS) {
    const primaryValue = String(primary?.strainLevels?.[key] || "");
    const validatorValue = String(validator?.strainLevels?.[key] || "");
    if (!primaryValue || !validatorValue) continue;
    comparedFields += 1;
    if (primaryValue !== validatorValue) {
      mismatches.push(`strain_${key}`);
      strainMismatchCount += 1;
    }
  }

  const mismatchCount = mismatches.length;
  const diffRate = comparedFields > 0 ? Number((mismatchCount / comparedFields).toFixed(4)) : 0;
  const majorMismatch =
    mismatches.includes("primary_type") ||
    mismatches.includes("instinctual_variant") ||
    centerMismatchCount >= 2 ||
    strainMismatchCount >= 3;
...
function evaluatePrimaryHardFail({ primarySnapshot, validatorSnapshot }) {
  const primary = primarySnapshot && typeof primarySnapshot === "object" ? primarySnapshot : buildCriticalSnapshot({});
  const validator = validatorSnapshot && typeof validatorSnapshot === "object" ? validatorSnapshot : buildCriticalSnapshot({});
  const reasons = [];
  const missingCritical = Array.isArray(primary?.missingCritical) ? primary.missingCritical : [];
  const missingStrainCategories = Array.isArray(primary?.missingStrainCategories)
    ? primary.missingStrainCategories
    : [];
  if (missingCritical.length) {
    reasons.push(`Primary missing critical fields: ${missingCritical.join(", ")}`);
  }
  if (missingStrainCategories.length) {
    reasons.push(`Primary strain sections incomplete: ${missingStrainCategories.join(", ")}`);
  }
  const diff = computeCriticalDiff(primary, validator);
  if (diff.majorMismatch) {
    reasons.push(`Major critical mismatch between primary and validator: ${diff.mismatches.join(", ")}`);
  }
  return {
...
function buildPrimaryRetryContext({ reasons, diff }) {
  const safeReasons = Array.isArray(reasons) ? reasons : [];
  const safeDiff = diff && typeof diff === "object" ? diff : {};
  const mismatchList = Array.isArray(safeDiff?.mismatches) ? safeDiff.mismatches : [];
  const lines = [
    "Retry objectives:",
    "1) Fill all critical fields: primaryType, instinctualVariant (sx/so/sp), action/feeling/thinking center labels, and all strain categories.",
    "2) Resolve major mismatches against validator evidence where present.",
  ];
  if (safeReasons.length) lines.push(`Hard-fail reasons: ${safeReasons.join(" | ")}`);
  if (mismatchList.length) lines.push(`Mismatched fields: ${mismatchList.join(", ")}`);
  return lines.join("\n");
}

function applyPrimaryCriticalOverrides(parsedData, snapshot) {
  if (!parsedData || typeof parsedData !== "object") return parsedData;
  const safeSnapshot = snapshot && typeof snapshot === "object" ? snapshot : null;
  if (!safeSnapshot) return parsedData;

  if (Number.isFinite(Number(safeSnapshot?.primaryType))) {
    const primaryType = Number(safeSnapshot.primaryType);
    parsedData.primaryType = primaryType;
    parsedData.core_type = primaryType;
  }
  if (safeSnapshot?.instinctualVariant) {
    parsedData.instinctualVariant = safeSnapshot.instinctualVariant;
  }

  const centers = safeSnapshot?.centers && typeof safeSnapshot.centers === "object" ? safeSnapshot.centers : {};
  const action = asUpperLevel(centers?.action);
  const feeling = asUpperLevel(centers?.feeling);
  const thinking = asUpperLevel(centers?.thinking);
  if (action || feeling || thinking) {
    parsedData.centerLabels = {
      action: action || parsedData?.centerLabels?.action || null,
      feeling: feeling || parsedData?.centerLabels?.feeling || null,
      thinking: thinking || parsedData?.centerLabels?.thinking || null,
    };
    parsedData.centers_of_expression = {
      ...(parsedData?.centers_of_expression && typeof parsedData.centers_of_expression === "object"
        ? parsedData.centers_of_expression
        : {}),
      action: parsedData.centerLabels.action,
      feeling: parsedData.centerLabels.feeling,
      thinking: parsedData.centerLabels.thinking,
    };
    parsedData.centersOfExpression = parsedData.centers_of_expression;
    parsedData.centerScores = {
      body: normalizeLevelLabel(parsedData.centerLabels.action),
      heart: normalizeLevelLabel(parsedData.centerLabels.feeling),
      head: normalizeLevelLabel(parsedData.centerLabels.thinking),
    };
  }

  const strainNarratives =
    safeSnapshot?.strainNarratives && typeof safeSnapshot.strainNarratives === "object"
      ? safeSnapshot.strainNarratives
      : {};
  const hasStrainNarratives = PRIMARY_CRITICAL_STRAIN_KEYS.some((key) => normalizeWhitespace(strainNarratives?.[key] || "").length);
  if (hasStrainNarratives) {
    parsedData.strainInterpretations = {
      ...(parsedData?.strainInterpretations && typeof parsedData.strainInterpretations === "object"
        ? parsedData.strainInterpretations
        : {}),
      ...Object.fromEntries(
        Object.entries(strainNarratives)
...
function buildPageRangeTextFromLookup(pageTextByNumber, range = {}) {
  const source = pageTextByNumber && typeof pageTextByNumber === "object" ? pageTextByNumber : {};
  const start = Number.isFinite(Number(range?.start)) ? Math.floor(Number(range.start)) : null;
  const end = Number.isFinite(Number(range?.end)) ? Math.floor(Number(range.end)) : null;
  if (!Number.isFinite(start) || !Number.isFinite(end) || start <= 0 || end < start) return "";
  const chunks = [];
  for (let page = start; page <= end; page += 1) {
    const text = normalizeWhitespace(source[page] || "");
    if (!text) continue;
...
function extractCenterScoresFromCenterSectionText(text) {
  const source = String(text || "").trim();
  if (!source) return null;
  const scores = {
    body: normalizeLevelLabel(
      extractQualitativeLevelFromText(source, ["Action Center of Expression"]),
    ),
    heart: normalizeLevelLabel(
      extractQualitativeLevelFromText(source, ["Feeling Center of Expression"]),
    ),
    head: normalizeLevelLabel(
      extractQualitativeLevelFromText(source, ["Thinking Center of Expression"]),
    ),
  };
  const nonNull = Object.values(scores).filter(Boolean).length;
  return nonNull >= 2 ? scores : null;
}
...
async function buildLocalTextFallbackParsedPayload({
  ...
  const centerPageText = buildPageRangeTextFromLookup(
    Object.fromEntries(extractedPages.map((page) => [page.pageNumber, page.extractedText])),
    CENTER_EXPRESSION_PAGE_RANGE,
  );
  const centerScoresFromCenterPages = extractCenterScoresFromCenterSectionText(centerPageText);
  const primaryType = pickPrimaryTypeFromScores(regexScores?.typeScores) || null;
  const centerScores = {
    body: normalizeLevelLabel(centerScoresFromCenterPages?.body || regexScores?.centerScores?.body),
    heart: normalizeLevelLabel(centerScoresFromCenterPages?.heart || regexScores?.centerScores?.heart),
    head: normalizeLevelLabel(centerScoresFromCenterPages?.head || regexScores?.centerScores?.head),
  };
  const strainLevels = {
...
export async function parsePdf(pdfBuffer, optionsOrId) {
  ...
  try {
    ...
    const centerPageText = buildPageRangeTextFromLookup(layoutAwarePageTextByNumber, CENTER_EXPRESSION_PAGE_RANGE);
    const centerScoresFromCenterPages = extractCenterScoresFromCenterSectionText(centerPageText);
    const prioritizedCenterScores = {
      body: normalizeLevelLabel(centerScoresFromCenterPages?.body || regexScores.centerScores.body),
      heart: normalizeLevelLabel(centerScoresFromCenterPages?.heart || regexScores.centerScores.heart),
      head: normalizeLevelLabel(centerScoresFromCenterPages?.head || regexScores.centerScores.head),
    };
    ...
    console.log("[parsePdf] Center extraction from pages 12-13", {
      action: prioritizedCenterScores.body,
      feeling: prioritizedCenterScores.heart,
      thinking: prioritizedCenterScores.head,
      usedCenterPageRange: Boolean(centerScoresFromCenterPages),
    });
    ...
    const primaryStructured = await extractPrimaryStructuredWithAzureOpenAi({
      openAiUrl,
      apiKey: process.env.AZURE_OPENAI_API_KEY,
      masterDocumentText,
    });
    const parsedData = primaryStructured && typeof primaryStructured === "object"
      ? primaryStructured
      : {};
    ...
    const centersOfExpression = {
      action: parseCenterLabel(semCenter.action) || parseCenterLabel(prioritizedCenterScores.body) || "HIGH",
      feeling: parseCenterLabel(semCenter.feeling) || parseCenterLabel(prioritizedCenterScores.heart) || "MEDIUM",
      thinking: parseCenterLabel(semCenter.thinking) || parseCenterLabel(prioritizedCenterScores.head) || "LOW",
      center_specific_styles: ["Externalised Action Center", "Externalised Feeling Center", "Internalised Thinking Center"],
    };
    parsedData.centerLabels = {
      action: centersOfExpression.action,
      feeling: centersOfExpression.feeling,
      thinking: centersOfExpression.thinking,
    };
    ...
    const primaryCriticalSnapshot = buildCriticalSnapshot(parsedData);
    console.log("[parsePdf] Primary critical snapshot prepared.", {
      parserVersion: PRIMARY_PARSER_VERSION,
      missingCritical: primaryCriticalSnapshot.missingCritical,
      missingStrainCategories: primaryCriticalSnapshot.missingStrainCategories,
      instinctualVariant: primaryCriticalSnapshot.instinctualVariant,
      primaryType: primaryCriticalSnapshot.primaryType,
    });
    ...
    const validatorCriticalSnapshot = buildCriticalSnapshot(parsedData);
    let primaryValidation = evaluatePrimaryHardFail({
      primarySnapshot: primaryCriticalSnapshot,
      validatorSnapshot: validatorCriticalSnapshot,
    });
    let selectedSource = "primary";
    let selectedPrimarySnapshot = primaryCriticalSnapshot;
    let retryAttempted = false;
    let retrySucceeded = false;
    let retryError = null;
    console.log("[parsePdf] Primary vs validator critical diff.", {
      diffRate: primaryValidation.diff.diffRate,
      mismatches: primaryValidation.diff.mismatches,
      hardFail: primaryValidation.hardFail,
      hardFailReasons: primaryValidation.reasons,
    });
    if (primaryValidation.hardFail && PRIMARY_VALIDATION_RETRY_LIMIT > 0) {
      retryAttempted = true;
      const retryContext = buildPrimaryRetryContext({
        reasons: primaryValidation.reasons,
        diff: primaryValidation.diff,
      });
      console.log("[parsePdf] Primary hard fail detected. Retrying primary extraction with focused prompt.", {
        retryLimit: PRIMARY_VALIDATION_RETRY_LIMIT,
        retryContext,
      });
      try {
        const retriedPrimaryStructured = await extractPrimaryStructuredWithAzureOpenAi({
          openAiUrl,
          apiKey: process.env.AZURE_OPENAI_API_KEY,
          masterDocumentText,
          retryContext,
        });
        const retriedPrimarySnapshot = buildCriticalSnapshot(retriedPrimaryStructured);
        const retryValidation = evaluatePrimaryHardFail({
          primarySnapshot: retriedPrimarySnapshot,
          validatorSnapshot: validatorCriticalSnapshot,
        });
        if (!retryValidation.hardFail) {
          retrySucceeded = true;
          primaryValidation = retryValidation;
          selectedPrimarySnapshot = retriedPrimarySnapshot;
          console.log("[parsePdf] Primary retry succeeded. Using primary as source of truth for critical fields.", {
            diffRate: retryValidation.diff.diffRate,
            mismatches: retryValidation.diff.mismatches,
          });
        } else {
          primaryValidation = retryValidation;
          selectedSource = "validator";
          console.log("[parsePdf] Primary retry still failed. Promoting validator parser output.", {
            hardFailReasons: retryValidation.reasons,
            mismatches: retryValidation.diff.mismatches,
          });
        }
      } catch (retryFailure) {
        selectedSource = "validator";
        retryError = String(retryFailure?.message || retryFailure);
        console.log("[parsePdf] Primary retry request failed. Promoting validator parser output.", {
          retryError,
        });
      }
    } else if (primaryValidation.hardFail) {
      selectedSource = "validator";
    }
    if (primaryValidation.hardFail) selectedSource = "validator";
    if (selectedSource === "primary") {
      applyPrimaryCriticalOverrides(parsedData, selectedPrimarySnapshot);
    }
    ...
    return {
      ...
      _parseDiagnostics: {
        ...
        primaryValidation: {
          primaryParserVersion: PRIMARY_PARSER_VERSION,
          validatorParserVersion: parserVersion,
          selectedSource,
          retryAttempted,
          retrySucceeded,
          retryError,
          ...
          hardFailReasons: primaryValidation.reasons,
          diffRate: primaryValidation.diff.diffRate,
          comparedFields: primaryValidation.diff.comparedFields,
          mismatchCount: primaryValidation.diff.mismatchCount,
          mismatches: primaryValidation.diff.mismatches,
        },
