...
function mapAttachedToLegacyPayload({
  ...
  return {
    ...
    reportContent: {
      documentSummary: `Processed ${pageSnapshots.length} pages using LlamaParse markdown/HTML + LLM parser.`,
...
export async function parsePdf(pdfBuffer, optionsOrId) {
  ...
  try {
    ...
    llamaRequiredForRun = !rawTextOverride;
    ...
    if (rawTextOverride) {
      ...
      try {
        const llamaParseMarkdown = await extractTextWithLlamaParse(pdfBuffer);
        extractedPages = buildOverridePages(llamaParseMarkdown, effectivePageCount);
        lastKnownExtractedPageCount = extractedPages.length;
        extractionMethod = "llamaparse_markdown";
        console.log("[parsePdf] Using LlamaParse markdown/HTML output for LLM parsing.", {
          pages: extractedPages.length,
          chars: llamaParseMarkdown.length,
          sourceFileName,
          parserVersion: PARSER_VERSION,
        });
        if (enableCanonicalRag) {
          canonicalRagContext = await buildCanonicalRagContext({
            enabled: true,
            rawText: llamaParseMarkdown,
            sourceFileName,
          });
          lastKnownCanonicalRag = canonicalRagContext;
          console.log(
            "[parsePdf] Canonical RAG context refreshed from LlamaParse markdown.",
        ...
        if (!allowLocalTextFallback) throw llamaParseError;

        console.log("[parsePdf] LlamaParse extraction failed; retrying through local full-text extraction fallback.", {
          details: String(llamaParseError?.message || llamaParseError),
          parserVersion: PARSER_VERSION,
        });
