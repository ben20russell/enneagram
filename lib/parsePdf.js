...
function configurePdfJsForNode(pdfjsLib) {
  ...
  try {
    if (pdfjsLib?.GlobalWorkerOptions && typeof pdfjsLib.GlobalWorkerOptions === "object") {
      // Force worker-free mode in Node/serverless to avoid worker file resolution failures.
...
function candidateApiVersions(preferredApiVersion) {
  const apiVersionsToTry = [
    ...
    "2024-02-15-preview", // Stable multimodal preview fallback
...
function buildChatPayload({ systemPrompt, userPrompt, imagesOrPdfDataUrl, model }) {
