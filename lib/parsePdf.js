...
async function rasterizePdfToImageDataUrls(pdfBuffer, { maxPages = 8, pageNumbers = null } = {}) {
  if (typeof globalThis.__parsePdfRasterizeHook === "function") {
    return await globalThis.__parsePdfRasterizeHook(pdfBuffer, { maxPages, pageNumbers });
  }
...
async function runScoreRescueViaChatImages({ endpoint, apiKey, apiVersion, deployment, pdfBuffer, reportContent }) {
  ...
  const payload = buildChatPayload({
    schema: SCORE_RESCUE_SCHEMA,
    systemPrompt: SCORE_RESCUE_PROMPT,
    userPrompt: "Read the charts and return only numeric score JSON.",
    imagesOrPdfDataUrl: imageDataUrls,
  });
  ...
  try {
    ...
    console.log("[parsePdf] image score rescue legacy endpoint failed, trying model-based chat endpoint", {
