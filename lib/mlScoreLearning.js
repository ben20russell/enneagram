...
function buildMlFeatures(parsedProfile) {
  const profile = normalizeOptionalObject(parsedProfile);
  ...
  return {
    primaryType: normalizeType(profile?.primaryType ?? profile?.typeNumber ?? profile?.type_number),
    instinctualVariant: normalizeInstinct(profile?.instinctualVariant),
    integrationLevel: normalizeIntegration(profile?.integrationLevel),
...
export function buildMlTrainingExamplesFromReportRows(rows, options = {}) {
  ...
  reportRows.forEach((row) => {
    ...
    const features = buildMlFeatures(parsedProfile);
    examples.push({
