...
test("parsedProfile strain levels in jsonb map LOW/MEDIUM/HIGH strings into numeric dashboard levels", () => {
  ...
  assert.equal(mapped.happiness, 100);
  assert.equal(mapped.vocational, 50);
  assert.equal(mapped.interpersonal, 0);
  assert.equal(mapped.physical, 100);
  assert.equal(mapped.environmental, 0);
  assert.equal(mapped.psychological, 50);
  assert.equal(mapped.overall, 50);
});
