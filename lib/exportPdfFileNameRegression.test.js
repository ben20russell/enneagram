...
test("export flow applies and restores document.title around print for PDF filename stability", () => {
  const source = read(reportScriptPath);

  assert.match(
    source,
    /const\s+previousDocumentTitle\s*=\s*document\.title;/,
    "Expected export flow to capture the existing document title before print.",
  );
  assert.match(
    source,
    /document\.title\s*=\s*exportTitle;/,
    "Expected export flow to apply exportTitle to document.title before print.",
  );
  assert.match(
    source,
    /document\.title\s*=\s*previousDocumentTitle;/,
    "Expected export flow to restore the original document.title after print cleanup.",
  );
});
