...
test("reparse-assigned-report parse call passes rawTextOverride and pageCountOverride when available", () => {
  ...
  assert.match(
    source,
    /parsePdf\(\s*pdfBuffer\s*,\s*\{[\s\S]*rawTextOverride[\s\S]*pageCountOverride[\s\S]*\}\s*\)/,
    "Expected reparse script parse options to include rawTextOverride and pageCountOverride.",
  );
});
