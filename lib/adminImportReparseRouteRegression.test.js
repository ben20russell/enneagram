...
test("admin import reparse route sanitizes downloaded report bytes before parse", () => {
  const source = read(reparseRoutePath);

  assert.match(
    source,
    /import\s+\{\s*(?:resolvePdfSanitizeFormFieldMode,\s*)?sanitizePdfForParsing\s*\}\s+from\s+["']\.\.\/\.\.\/\.\.\/\.\.\/lib\/pdfSanitize\.js["']/,
    "Expected reparse route to import shared PDF sanitization helper",
  );

  assert.match(
    source,
    /const\s+sanitizedPdf\s*=\s*await\s+sanitizePdfForParsing\(\s*pdfBuffer\s*,\s*\{/,
    "Expected reparse route to sanitize PDF bytes before parse",
  );

  assert.match(
    source,
    /parsePdf\(\s*sanitizedPdf\.buffer\s*,\s*\{/,
    "Expected reparse route to parse sanitized bytes",
  );
});
...
test("admin import reparse route uses safe parse options for serverless stability", () => {
  ...
  assert.match(
    ...
    "Expected reparse route parse call to disable image pipeline, disable image score rescue, and enable Python cross-checking",
  );

  assert.match(
    source,
    /buildMlExtractionLearningContext/,
    "Expected reparse route to build extraction-learning context before parse",
  );

  assert.match(
    source,
    /extractionLearningContext/,
    "Expected reparse route parse call to include extraction-learning context",
  );
});

test("admin import reparse route returns detected total pages in parse response", () => {
  const source = read(reparseRoutePath);

  assert.match(
    source,
    /parseDetectedTotalPages:\s*nextDiagnostics\?\.extraction\?\.detectedTotalPages\s*\?\?\s*null/,
    "Expected reparse route success payload to include detected total PDF pages",
  );
});
