...
test("hydration diagnostics payload exposes slot contract coverage and source audit metadata", () => {
  ...
  assert.match(
    source,
    /"instinctGoalSelfPres"[\s\S]*"instinctGoalSocial"[\s\S]*"instinctGoalOneOnOne"/,
    "Expected required slot contract to include instinct-goal hydration coverage.",
  );
  assert.match(
    source,
    /"teamImpactCopy"[\s\S]*"interdependenceCopy"[\s\S]*"overallStrainSummary"/,
    "Expected required slot contract to include expanded narrative hydration coverage beyond baseline slots.",
  );
