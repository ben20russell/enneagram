// INACTIVE ARCHIVE COPY: do not import; kept for records as requested.
...
// Force server bundles to keep the worker asset and point pdfjs at the packaged worker entry.
...
const SECTIONS_SCHEMA = {
  ...
  required: ["sections", "developmentExercisesText", "documentSummary"],
  additionalProperties: false,
};
...
Use stable snake_case sectionId values when possible. For INDIVIDUAL PROFESSIONAL / PRO reports, explicitly capture sections with ids:
- strategic_leadership
- team_dynamics
- decision_framework
- coaching_relationship
- feedback_matrix
Also return developmentExercisesText as one string block containing all development exercise wording in source order.
For pages 28-29 feedback guide content, the text appears in a visual grid with icons. Extract the text directly adjacent to the numbers 1 through 9.
Return strict JSON.`;
...
const MAX_RETRIES = 5;
const DEFAULT_PDF_IMAGE_RENDER_SCALE = 2.4;
...
function configurePdfJsForNode(pdfjsLib) {
  ...
  try {
    if (pdfjsLib?.GlobalWorkerOptions && typeof pdfjsLib.GlobalWorkerOptions === "object") {
      // Keep worker config deterministic in serverless runtimes and avoid traced file-path lookups.
      pdfjsLib.GlobalWorkerOptions.workerPort = null;
      pdfjsLib.GlobalWorkerOptions.workerSrc = "pdfjs-dist/legacy/build/pdf.worker.mjs";
    }
  } catch (error) {
...
function normalizeParsedShape(raw) {
  ...
  return {
    ...
    reportContent: {
      ...
      developmentExercisesText: raw?.reportContent?.developmentExercisesText ?? null,
...
async function rasterizePdfToImageDataUrls(
  pdfBuffer,
  { maxPages = 8, pageNumbers = null, scale = DEFAULT_PDF_IMAGE_RENDER_SCALE } = {},
) {
    ...
    for (const pageNumber of pagesToRender) {
      ...
      try {
        ...
        if (!Number.isFinite(effectiveScale) || effectiveScale <= 0.25) {
          effectiveScale = DEFAULT_PDF_IMAGE_RENDER_SCALE;
        }
...
function stripInlinePdfFooterNoise(value) {
  return String(value || "")
...
function isPdfFooterNoiseLine(value) {
  const line = normalizePdfLayerText(value);
  if (!line) return false;
  if (/^(?:copyright|©)\s*\d{4}(?:\s*[-–]\s*\d{4})?/i.test(line)) return true;
  if (
    /integrative\s+enneagram|integrative\s+enneagram\s+solutions|ben\s+russell|all\s+rights\s+reserved/i.test(line) &&
    /\b\d{1,3}\s+of\s+\d{1,3}\b/i.test(line)
  ) {
    return true;
  }
  if (/^\s*(?:page\s+)?\d{1,3}\s+of\s+\d{1,3}\s*$/i.test(line)) return true;
  return false;
}

function stripPdfFooterNoiseFromText(value) {
  const inlineStripped = stripInlinePdfFooterNoise(value);
  return normalizePdfLayerText(inlineStripped)
    .split(/\n+/)
    .map((line) => normalizePdfLayerText(line))
    .filter((line) => line && !isPdfFooterNoiseLine(line))
    .join("\n")
    .trim();
}

function sanitizePageKeyDataPoints(values) {
  return Array.from(
    new Set(
      (Array.isArray(values) ? values : [])
        .map((value) => stripPdfFooterNoiseFromText(value))
        .map((value) => normalizePdfLayerText(value))
        .filter((value) => value && !isPdfFooterNoiseLine(value))
...
async function extractPagesTextFromPdfLayer(pdfBuffer) {
  ...
  for (let pageNumber = 1; pageNumber <= Number(pdf.numPages || 0); pageNumber += 1) {
    ...
    const contentLines = lines.filter((line) => !isPdfFooterNoiseLine(line));
    const extractedText = stripPdfFooterNoiseFromText(contentLines.join("\n"));
    pages.push({
      pageNumber,
      heading: inferPageHeading(contentLines),
      extractedText: extractedText || null,
      keyDataPoints: sanitizePageKeyDataPoints(inferKeyDataPoints(contentLines)),
    });
  }
...
function normalizePageForMerge(raw) {
  ...
  const heading = raw?.heading == null ? null : normalizePdfLayerText(String(raw.heading));
  const extractedText = raw?.extractedText == null ? null : stripPdfFooterNoiseFromText(String(raw.extractedText));
  return {
    pageNumber,
    heading: heading && !isPdfFooterNoiseLine(heading) ? heading : null,
    extractedText: extractedText || null,
    keyDataPoints: sanitizePageKeyDataPoints(raw?.keyDataPoints),
  };
}
...
function pickPreferredPageText(layerText, modelText) {
  ...
  if (normalizedLayer) return normalizedLayer;
  return null;
}
...
function mergePagesWithPdfTextLayer(pagesPayload, layerPagesPayload) {
  ...
  const mergedPages = pageNumbers.map((pageNumber) => {
    ...
    const modelKeyDataPoints = sanitizePageKeyDataPoints(model?.keyDataPoints);
    const layerKeyDataPoints = sanitizePageKeyDataPoints(layer?.keyDataPoints);
    const mergedKeyDataPoints = modelKeyDataPoints.length ? modelKeyDataPoints : layerKeyDataPoints;
    return {
      pageNumber,
      heading: model?.heading || layer?.heading || null,
...
async function runSectionsPass({ endpoint, apiKey, apiVersion, deployment, pdfDataUrl }) {
  return runResponsesPass({
    ...
    userPrompt:
      "Return section-by-section extraction for the full report. Include PRO keys strategic_leadership, team_dynamics, decision_framework, coaching_relationship, and feedback_matrix when present. Return developmentExercisesText as one string block (not an array). For pages 28-29 feedback guide, treat it as a visual grid with icons and extract text adjacent to numbers 1 through 9.",
...
function collectReportTextForTemplateDetection(parsed) {
  ...
  if (normalized?.reportContent?.developmentExercisesText) {
    chunks.push(String(normalized.reportContent.developmentExercisesText));
  }
...
function buildSectionIdentity(section) {
  const id = normalizePdfLayerText(section?.sectionId || "").toLowerCase();
  if (id) return `id:${id}`;
  const title = normalizePdfLayerText(section?.sectionTitle || "").toLowerCase();
  const start = Number(section?.pageStart || 0);
  const end = Number(section?.pageEnd || 0);
  return `range:${start}-${end}|title:${title}`;
}
...
function chooseRicherSectionText(primary, secondary) {
  const a = normalizePdfLayerText(primary || "");
  const b = normalizePdfLayerText(secondary || "");
  if (!a && !b) return null;
  if (!a) return b || null;
  if (!b) return a || null;
...
function normalizeSectionId(value) {
  return normalizePdfLayerText(value || "")
    .toLowerCase()
    .replace(/\s+/g, "_");
}

function normalizeProSections(proSectionsRaw) {
  if (!proSectionsRaw || typeof proSectionsRaw !== "object") return [];
  ...
  return Object.entries(proSectionsRaw)
    .map(([rawId, raw]) => {
      const sectionId = normalizeSectionId(rawId);
      if (!sectionId) return null;
      const sectionTitle =
        normalizePdfLayerText(raw?.sectionTitle || defaultTitles[sectionId] || toTitleCaseFromSnakeCase(sectionId)) ||
        defaultTitles[sectionId] ||
        toTitleCaseFromSnakeCase(sectionId);

      if (typeof raw === "string") {
        const text = normalizePdfLayerText(raw || "") || null;
        if (!text) return null;
        return {
          sectionId,
          sectionTitle,
      ...
      const summary = normalizePdfLayerText(raw?.summary || "") || null;
      const fullText = normalizePdfLayerText(raw?.fullText || "") || null;
      const hasPageStart = Number.isInteger(pageStart) && pageStart > 0;
      const hasPageEnd = Number.isInteger(pageEnd) && pageEnd > 0;
      if (!hasPageStart && !hasPageEnd && !summary && !fullText) return null;
      return {
        sectionId,
        sectionTitle,
        pageStart: hasPageStart ? pageStart : null,
        pageEnd: hasPageEnd ? pageEnd : hasPageStart ? pageStart : null,
        summary,
        fullText,
      };
    })
...
function mergeProSectionsIntoSections(sections, proSectionsRaw) {
  const baseSections = Array.isArray(sections) ? sections : [];
  const proSections = normalizeProSections(proSectionsRaw);
  if (!proSections.length) return baseSections;

  const proById = new Map(
    proSections
      .map((section) => [normalizeSectionId(section?.sectionId), section])
      .filter(([id]) => Boolean(id)),
  );
  const merged = baseSections.map((baseSection) => {
    const sectionId = normalizeSectionId(baseSection?.sectionId);
    if (!sectionId || !proById.has(sectionId)) return baseSection;
    const proSection = proById.get(sectionId);
    proById.delete(sectionId);

    const baseStart = Number(baseSection?.pageStart);
    const baseEnd = Number(baseSection?.pageEnd);
    const hasBaseStart = Number.isInteger(baseStart) && baseStart > 0;
    const hasBaseEnd = Number.isInteger(baseEnd) && baseEnd > 0;

    return {
      ...baseSection,
      sectionId: baseSection?.sectionId || proSection?.sectionId || null,
      sectionTitle: baseSection?.sectionTitle || proSection?.sectionTitle || null,
      pageStart: hasBaseStart ? baseStart : proSection?.pageStart ?? null,
      pageEnd: hasBaseEnd ? baseEnd : proSection?.pageEnd ?? (hasBaseStart ? baseStart : null),
      summary: chooseRicherSectionText(baseSection?.summary, proSection?.summary),
      fullText: chooseRicherSectionText(baseSection?.fullText, proSection?.fullText),
    };
  });

  const keys = new Set(merged.map((section) => buildSectionIdentity(section)));
  proById.forEach((section) => {
    const key = buildSectionIdentity(section);
...
function mergeReportContent(baseParsed, pagesPayload, sectionsPayload) {
  ...
  const sectionsWithProCoverage = mergeProSectionsIntoSections(
    Array.isArray(sectionsPayload?.sections) ? sectionsPayload.sections : [],
    sectionsPayload?.proSections,
  );
  const sections = mergeSectionsWithPageFallback(
    pages,
    sectionsWithProCoverage,
  );
  const developmentExercisesText = stripPdfFooterNoiseFromText(
    sectionsPayload?.developmentExercisesText ?? merged?.reportContent?.developmentExercisesText ?? "",
  ) || null;
...
async function runPagesPassViaChatImages({
  ...
  const resolvedScale = Number.isFinite(resolvedScaleRaw) && resolvedScaleRaw > 0
    ? resolvedScaleRaw
    : DEFAULT_PDF_IMAGE_RENDER_SCALE;
...
async function runSectionsFromPagesViaChat({ endpoint, apiKey, apiVersion, deployment, pagesPayload }) {
  ...
  const userPrompt = [
    ...
    "Use snake_case section ids. Include strategic_leadership, team_dynamics, decision_framework, coaching_relationship, and feedback_matrix for PRO reports when present.",
    "Return developmentExercisesText as a single string block containing all development exercises in source order (do not return an array).",
    "For pages 28-29 feedback guide, treat it as a visual grid with icons and extract text directly adjacent to numbers 1 through 9.",
    compactPagesText,
  ].join("\n");
...
export async function parsePdf(input, options = {}) {
  ...
  if (!hasAnyPages && Array.isArray(deterministicPagesPayload?.pages) && deterministicPagesPayload.pages.length > 0) {
    parsed = mergeReportContent(parsed, deterministicPagesPayload, {
      ...
      developmentExercisesText: parsed?.reportContent?.developmentExercisesText ?? null,
  ...
  // Score rescue is force-disabled to keep parsing deterministic and avoid multi-scale rerender loops.
  const missingNumericCharts = false;
  if (missingNumericCharts) {
    ...
    pushStep("image_score_rescue_skipped", {
      reason: "score_rescue_forced_disabled",
    });
  }
  ...
  const parsePages = extractedPages;
  const parseMinExpectedPages = diagnostics.extraction.minExpectedPages;
  const parsedData = {
    core_type: normalized?.core_type ?? normalized?.primaryType ?? null,
    instinct: normalized?.instinct ?? normalized?.instinctualVariant ?? null,
  };
  const hasMinimumPages = parsePages >= parseMinExpectedPages;
  const hasCoreResults = parsedData.core_type !== null && parsedData.instinct !== null;
  ...
  if (!hasMinimumPages) {
    ...
    diagnostics.incompleteReason = `Extracted ${parsePages} pages, expected at least ${parseMinExpectedPages}`;
    pushError("parse_incomplete_low_page_coverage", diagnostics.incompleteReason);
  } else {
    diagnostics.isComplete = true;
    diagnostics.incompleteReason = null;
    if (!hasCoreResults) {
      pushWarning(
        "parse_complete_missing_core_results",
        "Page coverage complete; core top-level results (core_type or instinct) are missing and should be reviewed.",
      );
    }
  ...
  let _parseStatus = "incomplete";
  let _reviewStatus = "needs_review";
  if (parsePages >= parseMinExpectedPages) {
