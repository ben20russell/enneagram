...
test("parsePdf uses direct local text extraction for PDF parsing", () => {
  const source = read(parsePdfPath);

  assert.match(source, /Using text-only PDF extraction for LLM parsing/i);
  assert.match(source, /extractPdfPagesWithPython\(pdfBuffer\)/);
  assert.match(source, /No extractable PDF text found\./i);
  assert.doesNotMatch(source, /Agentic pipeline failed; retrying through local full-text extraction fallback\./i);
});
