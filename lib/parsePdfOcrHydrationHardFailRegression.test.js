...
test("parsePdf re-routes to local OCR-aware extraction when the agentic pipeline fails", () => {
  const source = read(parsePdfPath);

  assert.match(source, /Agentic pipeline failed; retrying through local full-text extraction fallback\./i);
