...
test("parsePdf re-routes to local OCR-aware extraction when LlamaParse fails", () => {
  const source = read(parsePdfPath);

  assert.match(source, /LlamaParse extraction failed; retrying through local full-text extraction fallback\./i);
