...
test("dashboard script populates Client Reports dropdown from report-active admin payload", () => {
  ...
  assert.match(
    script,
    /setClientReportSwitchVisible\(\s*isAdmin\s*&&\s*adminClientReports\.length\s*>\s*0\s*\)/,
    "Expected Client Reports selector visibility to be admin-only with available client report data",
  );
});
