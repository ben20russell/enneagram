...
test("dashboard renders a separate Client Reports dropdown control", () => {
  ...
  assert.match(
    html,
    /for="reportSelector">Example Reports</,
    "Expected example report selector label to use plural copy: Example Reports.",
  );

  assert.doesNotMatch(
    html,
    /id="reportSelectorMyReportOption"/,
    "Expected My Report option to be removed from Example Reports selector.",
  );
