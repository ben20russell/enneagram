...
test("parsePdf calls prebuilt-layout and returns diagnostics-driven payload fields", () => {
  const source = read(parsePdfPath);
  ...
  assert.match(
    source,
    /const\s+regexScores\s*=\s*extractScoresFromText\s*\(\s*masterDocumentText\s*\)/,
    "Expected parsePdf to run deterministic regex score extraction from raw text.",
  );

  assert.match(
    source,
    /parsedData\.reportContent\s*=\s*\{[\s\S]*sections:\s*\[\.\.\.existingSections,\s*\.\.\.flattenedProSections\]/,
    "Expected parsePdf to flatten PRO sections into reportContent.sections.",
  );

  assert.match(
    source,
    /rawScores:\s*rawScoreSnapshot/,
    "Expected parsePdf diagnostics to include all raw score fields via rawScoreSnapshot.",
  );
