...
test("parsePdf uses Azure Document Intelligence client initialization", () => {
  ...
  assert.match(
    source,
    /new\s+DocumentAnalysisClient\s*\(\s*endpoint\s*,\s*new\s+AzureKeyCredential\s*\(\s*key\s*\)\s*\)/,
...
test("parsePdf calls prebuilt-layout and returns parse status fields", () => {
  ...
  assert.match(
    source,
    /beginAnalyzeDocument\s*\(\s*["']prebuilt-layout["']\s*,\s*pdfBuffer\s*\)/,
  ...
  assert.match(
    source,
    /parsePages:\s*pageList\.length/,
    "Expected parsePdf success payload to include parsePages from the normalized analyzed pages length.",
  );

  assert.match(
    source,
    /reviewStatus:\s*['"]needs_review['"]/,
    "Expected parsePdf error payload to include reviewStatus='needs_review'.",
  );
});
