...
test("admin page auth menu link opens in a new tab while preserving opener for close-window flow", () => {
  const html = read(reportHtmlPath);

  assert.match(
    html,
    /id="authAdminPageLink"[\s\S]*?target="_blank"/i,
    "Expected Admin Page menu link to open in a new tab and keep current page unchanged.",
  );

  assert.doesNotMatch(
    html,
    /id="authAdminPageLink"[\s\S]*?rel="noopener noreferrer"/i,
    "Expected Admin Page menu link to retain opener so Close Window can work from admin-import.",
  );
});
