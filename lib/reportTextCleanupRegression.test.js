...
function loadTextCleanupFns() {
  ...
  const pieces = [
    extractFunctionSource(scriptSource, "normalizeColonSpacing"),
    ...
    extractFunctionSource(scriptSource, "normalizeCriticalDevelopmentExerciseText"),
    extractFunctionSource(scriptSource, "buildDevExercisePathHtml"),
    "globalThis.__exports = { normalizeColonSpacing, sanitizeSnippet, ensureSentenceStartsCapitalized, formatOptionalText, buildDevExercisePathHtml };",
  ];
...
test("sanitizeSnippet deglues core-pattern run-on words and strips letter-spaced page markers", () => {
  const { sanitizeSnippet } = loadTextCleanupFns();
  const cleaned = sanitizeSnippet(
    "Youtendtoquicklymentallyassesspeopleaseitherstrongorweakandthentreatpeopleaccordingtothatassessment. P a g e 6 o f 4 2",
    null,
  );

  assert.equal(
    cleaned,
    "You tend to quickly mentally assess people as either strong or weak and then treat people according to that assessment.",
  );
  assert.doesNotMatch(cleaned, /\bpage\b|\b6\b|\b42\b/i);
});
...
test("text cleanup enforces a single space after colons across sanitized and optional text values", () => {
  const { sanitizeSnippet, formatOptionalText } = loadTextCleanupFns();

  assert.equal(
    sanitizeSnippet("Externalised Action:As an Ennea 7, your body is active.", null),
    "Externalised Action: As an Ennea 7, your body is active.",
  );
  assert.equal(
    sanitizeSnippet("Internalised Thinking:\u200BAs an Ennea 7 your mind is active.", null),
    "Internalised Thinking: As an Ennea 7 your mind is active.",
  );
  assert.equal(
    formatOptionalText("Externalised Feeling:As you resonate with optimism.", "Not detected"),
    "Externalised Feeling: As you resonate with optimism.",
  );
});
...
test("development exercise cards render long multi-step text as bullet list items", () => {
  const { buildDevExercisePathHtml } = loadTextCleanupFns();
  const html = buildDevExercisePathHtml([
    {
      title: "Exercise 1",
      text:
        "Develop openness to others' input. Practice compassion and receptivity. Connect with vulnerability to become more resilient.",
    },
  ]);

  assert.match(html, /<ul class="dev-item-list">/);
  assert.match(html, /<li>Develop openness to others&#39; input\.<\/li>/);
  assert.match(html, /<li>Practice compassion and receptivity\.<\/li>/);
  assert.match(html, /<li>Connect with vulnerability to become more resilient\.<\/li>/);
  assert.doesNotMatch(html, /<p>Develop openness to others&#39; input\./);
});
...
test("dashboard render helpers normalize capitalization for text and html copy", () => {
  ...
  assert.match(
    script,
    /const\s+leadingWhitespace\s*=\s*text\.match\(\s*\/\^\\s\+\/\s*\)\?\.\[0\]\s*\|\|\s*""[\s\S]*node\.nodeValue\s*=\s*`\$\{leadingWhitespace\}\$\{normalizedText\}`;/,
    "Expected HTML copy normalization to preserve leading whitespace between inline tags and adjacent text.",
  );
});
