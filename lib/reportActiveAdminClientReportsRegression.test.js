...
test("report-active API scopes client report listing to uploaded admin-import reports", () => {
  ...
  assert.match(
    ...
    "Expected report-active route to load top-level enneagram_type for hydration fallback.",
  );
});
...
test("report-active API no longer hard-gates assigned readiness on fully populated chart scores", () => {
  ...
  assert.match(
    routeSource,
    /const\s+hasCoreIdentity\s*=\s*Boolean\(\s*parsedProfile\?\.primaryType\s*\|\|\s*parsedProfile\?\.typeName\s*\|\|\s*fallbackPrimaryType\s*!=\s*null\s*\)/,
