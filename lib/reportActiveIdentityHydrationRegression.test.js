...
test("report-active identity hydration falls back to top-level enneagram_type when parsed identity is missing", () => {
  const getIngestedDashboardContext = loadGetIngestedDashboardContext();
  ...
  assert.equal(hydrated?.detectedType, "9");
  assert.equal(hydrated?.detectedTypeSource, "reports.enneagram_type");
});
