...
test("admin import success actions no longer include a return-to-report button", () => {
  const source = read(adminImportFormPath);

  assert.doesNotMatch(
    source,
    /data-testid="admin-import-return-report"/,
    "Expected return-to-report button to be removed from admin import success actions",
  );

  assert.doesNotMatch(
    source,
    /function\s+handleReturnToReport\s*\(/,
    "Expected return-to-report handler to be removed from admin import form",
  );
});
