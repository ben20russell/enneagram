...
function loadParsingFns() {
  ...
  const pieces = [
    ...
    "globalThis.__exports = { sanitizeSnippet, cleanPdfExtractedValue, extractStrainQualitativeWriteups, extractTeamStageSnippet, extractTeamStageBreakdownFromLegacyFields, extractPdfPageTextFromItems };",
  ];
...
test("cleanPdfExtractedValue strips cid artifacts from extracted narrative text", () => {
  const { cleanPdfExtractedValue } = loadParsingFns();
  const source = [
    "Ben your perceived level of Vocational strain is HIGH.",
    "(cid:4)(cid:42)(cid:43)(cid:52)(cid:45)(cid:36)",
    "You stay focused on outcomes and keep delivery moving.",
  ].join(" ");

  const cleaned = cleanPdfExtractedValue(source);

  assert.doesNotMatch(
    String(cleaned || ""),
    /\(cid:\d+\)/i,
    "Expected cid extraction artifacts to be removed from cleaned narrative text.",
  );
  assert.match(
    String(cleaned || ""),
    /focused on outcomes and keep delivery moving/i,
    "Expected cleaned narrative text to retain the readable guidance sentence.",
  );
});
...
test("extractTeamStageSnippet prefers explicit stage block over interleaved overview bleed in storming extraction", () => {
  const { extractTeamStageSnippet } = loadParsingFns();
  const source = [
    "Some teams may get stuck in storming for extended periods of time.",
    "PERFORMING: STORMING: Although not all teams reach the performing stage, high performance teams are interdependent units.",
    "Your Ennea Type and Team Stages FORMING Ben, during the forming stage of team development, you are likely to:",
    "STORMING As an Ennea 8, during the storming phase of team development others are likely to experience you as:",
    "● Forthright, direct and willing to talk about issues",
    "● Comfortable with honest and intense conversations",
    "NORMING During the Norming phase of team development you are likely to:",
  ].join(" ");

  const storming = extractTeamStageSnippet(source, "Storming", ["Norming", "Performing"]);

  assert.match(
    String(storming || ""),
    /As an Ennea 8, during the storming phase of team development/i,
    "Expected Storming extraction to prioritize the explicit stage block over preceding overview bleed.",
  );
  assert.doesNotMatch(
    String(storming || ""),
    /^Although not all teams reach the performing stage/i,
    "Expected Storming extraction to avoid the interleaved performing-overview fragment.",
  );
});

test("extractTeamStageSnippet strips trailing cid footer noise from stage snippets", () => {
  const { extractTeamStageSnippet } = loadParsingFns();
  const source = [
    "PERFORMING In teams that reach this stage of team development, you are likely to:",
    "● Hold people accountable for their commitments and delivery",
    "● Willingly take responsibilities on that you see as important priorities",
    "(cid:4)(cid:42)(cid:43)(cid:52)(cid:45)(cid:36)(cid:34)(cid:35)(cid:47)",
  ].join(" ");

  const performing = extractTeamStageSnippet(source, "Performing", []);

  assert.match(
    String(performing || ""),
    /In teams that reach this stage of team development/i,
    "Expected Performing extraction to keep stage-specific guidance.",
  );
  assert.doesNotMatch(
    String(performing || ""),
    /\(cid:\d+\)/i,
    "Expected Performing extraction to remove cid footer artifacts.",
  );
});
