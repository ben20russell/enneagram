...
test("admin import route sanitizes report PDFs before parse calls", () => {
  const source = read(adminImportRoutePath);

  assert.match(
    source,
    /import\s+\{\s*(?:resolvePdfSanitizeFormFieldMode,\s*)?sanitizePdfForParsing\s*\}\s+from\s+["']\.\.\/\.\.\/\.\.\/lib\/pdfSanitize\.js["']/,
    "Expected admin import route to import shared PDF sanitization helper",
  );

  assert.match(
    source,
    /const\s+sanitizedPdf\s*=\s*await\s+sanitizePdfForParsing\(\s*pdfBuffer\s*,\s*\{/,
    "Expected admin import route to sanitize downloaded PDF bytes before parsing",
  );

  assert.match(
    source,
    /parsePdf\(\s*sanitizedPdf\.buffer\s*,\s*\{/,
    "Expected admin import parse flow to parse sanitized PDF bytes",
  );
});
...
test("admin import route uses safe parse options for serverless stability", () => {
  const source = read(adminImportRoutePath);

  assert.match(
    source,
    /parsePdf\(\s*sanitizedPdf\.buffer\s*,\s*\{[\s\S]*disableImagePipeline:\s*true[\s\S]*disableImageScoreRescue:\s*true[\s\S]*enablePythonCrossCheck:\s*true[\s\S]*\}\s*\)/,
    "Expected admin import parse calls to disable image pipeline, disable image score rescue, and enable Python cross-checking in serverless routes",
  );

  assert.match(
    source,
    /buildMlExtractionLearningContext/,
    "Expected admin import parse flow to prepare extraction-learning context before parsing",
  );

  assert.match(
    source,
    /extractionLearningContext/,
    "Expected admin import parse calls to pass extraction-learning context into parsePdf",
  );
});

test("admin import route reparse response includes detected total pages", () => {
  const source = read(adminImportRoutePath);

  assert.match(
    source,
    /parseDetectedTotalPages:\s*nextDiagnostics\?\.extraction\?\.detectedTotalPages\s*\?\?\s*null/,
    "Expected admin-import reparse response payload to include detected total PDF pages",
  );
});
