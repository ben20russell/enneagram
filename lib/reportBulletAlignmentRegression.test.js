...
test("global bullet rows keep text top-aligned while centering marker to the first line", () => {
  ...
  assert.match(
    html,
    /\.core-pattern-inline-item::before\{[^}]*top:calc\(\(\(1em\s*\*\s*var\(--lh-body\)\)\s*-\s*var\(--bullet-size\)\)\s*\/\s*2\)[^}]*\}/,
    "Expected inline core-pattern bullets to vertically center their marker to the first text line.",
  );
});
