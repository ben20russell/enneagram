...
test("core pattern card renders a dedicated 3-bullet container", () => {
  ...
  assert.match(
    html,
    /\.core-pattern-row-marker\{[^}]*width:20px[^}]*height:20px[^}]*font-size:9px[^}]*\}/,
    "Expected core pattern leading bullet marker to render smaller than the shared bullet marker.",
  );
