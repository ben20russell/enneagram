...
test("admin import typography follows admin review queue defaults", () => {
  const source = read(adminImportFormPath);

  assert.doesNotMatch(
    source,
    /const\s+DASHBOARD_SANS_FONT_FAMILY\s*=/,
    "Expected admin import page to avoid defining a custom sans font stack so it matches admin review typography",
  );

  assert.doesNotMatch(
    source,
    /const\s+DASHBOARD_DISPLAY_FONT_FAMILY\s*=/,
    "Expected admin import page to avoid defining a custom display font stack so it matches admin review typography",
  );

  assert.doesNotMatch(
    source,
    /data-testid="admin-import-page"[\s\S]*?fontFamily:/,
    "Expected admin import page container to avoid overriding font family",
  );

  assert.doesNotMatch(
    source,
    /data-testid="admin-import-title"[\s\S]*?fontFamily:/,
    "Expected admin import title to avoid overriding font family",
  );

  assert.match(
    source,
    /data-testid="admin-import-title"[\s\S]*?style=\{\{\s*margin:\s*0\s*\}\}/,
    "Expected admin import title to use the same zero-margin heading treatment as admin review title",
  );
});

test("admin import page left-aligns content within a centered review-width column", () => {
  const source = read(adminImportFormPath);

  assert.match(
    source,
    /data-testid="admin-import-page"[\s\S]*?maxWidth:\s*"980px"/,
    "Expected admin import page container to use the same centered content width as admin review",
  );

  assert.match(
    source,
    /data-testid="admin-import-page"[\s\S]*?margin:\s*"0 auto"/,
    "Expected admin import page container to remain centered on the page",
  );
