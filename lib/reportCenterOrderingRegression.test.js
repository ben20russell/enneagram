...
test("center wheel arc labels use padded viewBox framing and path-length fitting", () => {
  const script = read(reportJsPath);
  ...
  assert.match(
    script,
    /const\s+labelTextLength\s*=\s*Math\.max\(\s*60\s*,\s*labelArcLength\s*-\s*24\s*\)\s*;/,
    "Expected center wheel labels to derive a bounded text length from each arc path length.",
  );

  assert.match(
    script,
    /<textPath[^>]*textLength="\$\{labelTextLength\}"[^>]*lengthAdjust="spacing"/,
    "Expected center wheel arc labels to opt into path text-length fitting without glyph distortion.",
  );
});
