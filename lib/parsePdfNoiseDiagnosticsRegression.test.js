...
test("python report extractor strips cid OCR artifacts before metadata regex extraction", () => {
  const source = read(pythonExtractorPath);

  assert.match(
    source,
    /CID_ARTIFACT_PATTERN|strip_cid_artifacts/i,
    "Expected extract_report_pdf.py to define explicit cid artifact cleanup.",
  );

  assert.match(
    source,
    /normalize\([\s\S]*strip_cid_artifacts/i,
    "Expected normalize() in extract_report_pdf.py to remove cid artifacts before regex extraction.",
  );
});
