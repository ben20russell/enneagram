...
test("admin review panel exposes core identity grading controls and submits them", () => {
  ...
  assert.match(
    ...
    "Expected admin review panel to expose a read-only core-identity stretch point value.",
  );
  assert.match(
    source,
    /data-testid="admin-review-core-release-point-value"/,
    "Expected admin review panel to expose a read-only core-identity release point value.",
  );
  assert.doesNotMatch(
    source,
    /data-testid="admin-review-core-stretch-point"[\s\S]*?<select/s,
    "Expected stretch point dropdown input to be removed from admin review core identity controls.",
  );
  assert.doesNotMatch(
    source,
    /data-testid="admin-review-core-release-point"[\s\S]*?<select/s,
    "Expected release point dropdown input to be removed from admin review core identity controls.",
  );
  assert.match(
    source,
...
test("admin review panel derives release and stretch points from canonical main type mapping", () => {
  ...
  assert.match(
    source,
    /setCoreIdentity\(\(prev\)\s*=>\s*\(\{[\s\S]*stretchPoint:\s*canonicalPoints\?\.stretch[\s\S]*releasePoint:\s*canonicalPoints\?\.release/s,
    "Expected main-type updates to auto-apply canonical stretch/release points in component state.",
  );
  assert.match(
    source,
