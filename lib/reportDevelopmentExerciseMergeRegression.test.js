...
function loadDevelopmentExerciseFunctions() {
  ...
  const pieces = [
    ...
    extractFunctionSource(scriptSource, "normalizeDevelopmentExerciseGridItems"),
    "globalThis.__exports = { mergeDevelopmentExercises, normalizeDevelopmentExerciseGridItems };",
  ];
...
test("normalizeDevelopmentExerciseGridItems removes duplicate exercises and re-numbers titles", () => {
  const { normalizeDevelopmentExerciseGridItems } = loadDevelopmentExerciseFunctions();
  const normalized = normalizeDevelopmentExerciseGridItems([
    { title: "Exercise 3", text: "Practice one slower breath before responding in conflict." },
    { title: "Exercise 5", text: "Practice one slower breath before responding in conflict." },
    { title: "Exercise 9", text: "Name one body signal before reacting." },
  ]);

  assert.equal(Array.isArray(normalized), true);
  assert.equal(normalized.length, 2);
  assert.equal(String(normalized[0]?.title || ""), "Exercise 1");
  assert.equal(String(normalized[1]?.title || ""), "Exercise 2");
  assert.match(String(normalized[0]?.text || ""), /slower breath before responding/i);
  assert.match(String(normalized[1]?.text || ""), /body signal before reacting/i);
});
