...
function loadDevelopmentExerciseFunctions() {
  ...
  const pieces = [
    ...
    extractFunctionSource(scriptSource, "normalizeDevelopmentExerciseCarouselItems"),
    "globalThis.__exports = { mergeDevelopmentExercises, normalizeDevelopmentExerciseCarouselItems };",
  ];
...
test("normalizeDevelopmentExerciseCarouselItems removes duplicate exercises and re-numbers titles", () => {
  const { normalizeDevelopmentExerciseCarouselItems } = loadDevelopmentExerciseFunctions();
  const normalized = normalizeDevelopmentExerciseCarouselItems([
    { title: "Exercise 3", text: "Practice one slower breath before responding in conflict." },
