...
test("client report ingestion avoids invalidating active token when duplicate selector events fire", () => {
  const script = read(reportScriptPath);

  assert.match(
    script,
    /async\s+function\s+ingestAssignedReportIntoDashboard\s*\(\s*data\s*\)\s*\{[\s\S]*?if\s*\(\s*assignedReportIngested\s*\)\s*\{[\s\S]*?return;[\s\S]*?const\s+ingestionToken\s*=\s*activeAssignedIngestionToken\s*\+\s*1\s*;/,
    "Expected ingest flow to short-circuit duplicate events before incrementing the active ingestion token.",
  );
});
