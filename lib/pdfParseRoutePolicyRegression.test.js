...
test("pdf parse route passes non-blocking completeness policy and large-doc image guard options", () => {
  ...
  assert.match(
    source,
    /parsePdf\(\s*buffer\s*,\s*\{[\s\S]*imagePrimaryFullDocMaxPages:\s*routeImagePageLimit[\s\S]*requireChartScoresForComplete:\s*false[\s\S]*\}\s*\)/,
    "Expected pdf parse route to pass large-doc image guard and disable strict chart-score completeness blocking",
  );
});
