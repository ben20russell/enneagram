...
test("dashboard narrative cleanup route includes retry policy and section-boundary guards", () => {
  ...
  assert.match(
    routeSource,
    /const INSTINCT_FOREIGN_REFERENCE_PATTERNS\s*=\s*Object\.freeze\(\{/,
    "Expected dashboard cleanup route to define explicit foreign-instinct spillover patterns for guard logic.",
  );
  assert.match(
    routeSource,
    /function applyInstinctFieldIsolationGuard\(/,
    "Expected dashboard cleanup route to expose a deterministic instinct-field isolation guard.",
  );
  assert.match(
    routeSource,
    /applyInstinctFieldIsolationGuard\(\s*resolvedPayload,\s*guardedInputPayload\s*\)/,
    "Expected dashboard cleanup route to down-rank contaminated instinct fields before responding to the client.",
  );
  assert.match(
    routeSource,
    /instinctFieldGuard:\s*\{\s*downRankedFields:/,
    "Expected dashboard cleanup route responses to include instinct-field guard diagnostics.",
  );
});
