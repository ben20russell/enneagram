...
test("parsePdf can route PDF bytes through python Docling markdown extraction before LLM parsing", () => {
  const source = read(parsePdfPath);

  assert.match(
    source,
    /extractDashboardVerificationWithPython\(\{[\s\S]*pdfBuffer[\s\S]*sourceFileName[\s\S]*\}[\s\S]*markdown/i,
    "Expected parsePdf to read markdown from python verification payload when available.",
  );

  assert.match(
    source,
    /python_docling_markdown|docling_markdown/,
    "Expected parsePdf extraction diagnostics to expose docling markdown extraction source.",
  );
});
