...
test("parsePdf hydrates attachment page snapshots with local text extraction for deterministic section parsing", () => {
  const source = read(parsePdfPath);

  assert.match(
    source,
    /if\s*\(\s*[\s\S]*?extractionMethod\s*===\s*["']attached_pdf["'][\s\S]*?\)\s*\{[\s\S]*?extractPdfPagesWithPython\(pdfBuffer\)/,
    "Expected parsePdf to attempt local page text extraction inside attached-pdf success flow.",
  );
});
