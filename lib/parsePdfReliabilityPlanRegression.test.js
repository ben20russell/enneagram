...
test("parsePdf performs text extraction before LLM structuring", () => {
  const source = read(parsePdfPath);

  assert.doesNotMatch(
    source,
    /const\s+stageOneLayoutExtraction\s*=|const\s+repairedHtml\s*=\s*await\s+agenticOcrRepair\(/,
    "Expected parsePdf to avoid layout/image parsing and agentic OCR repair before structuring.",
  );

  assert.match(
    source,
    /extractPdfPagesWithPython\([\s\S]*?extractStructuredJsonFromRawText\(/,
    "Expected parsePdf to extract PDF page text before LLM structuring.",
  );
});

test("parsePdf records text-only extraction source in diagnostics", () => {
  const source = read(parsePdfPath);

  assert.match(
    source,
    /extractionMethod\s*=\s*"local_text_only"/,
    "Expected parsePdf extraction diagnostics to identify the text-only path.",
  );

  assert.doesNotMatch(
    source,
    /agentic_ocr_repaired_html|layout_html_python/,
    "Expected parsePdf diagnostics to avoid staged layout/OCR extraction labels.",
  );
});
