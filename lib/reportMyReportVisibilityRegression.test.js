...
test("dashboard uses assigned-report availability for My Report option visibility", () => {
  ...
  assert.match(reportScript, /if\s*\(hasAssignedReportAvailable\)\s*\{\s*ingestAssignedReportIntoDashboard\(data\)/, "Expected ingestion path to run when assigned report is available");
