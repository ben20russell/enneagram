...
test("assigned report type inference recognizes MAIN TYPE # metadata markers", () => {
  const script = read(reportJsPath);

  assert.match(
    script,
    /source:\s*"mainTypeHash"/,
    "Expected PDF type inference to explicitly support MAIN TYPE # style markers.",
  );

  assert.match(
    script,
    /source:\s*"mainTypeLoose"/,
    "Expected PDF type inference to include a loose Main Type fallback pattern for OCR drift.",
  );
});
