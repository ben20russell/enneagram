...
test("growth section exposes a dedicated DevExercise component shell", () => {
  ...
  assert.match(
    ...
    "Expected Development Exercise library to render as a carousel shell.",
  );

  assert.match(
    html,
    /id="developmentExerciseSlide"/,
    "Expected Development Exercise carousel to expose a dedicated slide container.",
  );

  assert.match(
    html,
    /id="developmentExercisesPrev"/,
    "Expected Development Exercise carousel to expose a previous-slide control.",
  );

  assert.match(
    html,
    /id="developmentExercisesNext"/,
    "Expected Development Exercise carousel to expose a next-slide control.",
  );
...
test("report render flow hydrates DevExercise component from integration and strain context", () => {
  ...
  assert.match(
    script,
    /setDevelopmentExerciseCarouselItems\(/,
    "Expected render flow to hydrate carousel items for Development Exercise library.",
  );

  assert.match(
    script,
    /setupDevelopmentExerciseCarouselControls\(\)/,
    "Expected lifecycle setup to bind Development Exercise carousel controls.",
  );
