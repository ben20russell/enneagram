...
test("admin review page includes a link button to admin import", () => {
  ...
  assert.match(
    source,
    /<section[\s\S]*?data-testid="admin-review-header-row"[\s\S]*?<Link[\s\S]*?data-testid="admin-review-import-link-button"[\s\S]*?href="\/admin-import"[\s\S]*?>[\s\S]*?Open Admin Import[\s\S]*?<\/Link>/,
    "Expected admin review panel to place the import button in the top-right header row.",
  );
});
