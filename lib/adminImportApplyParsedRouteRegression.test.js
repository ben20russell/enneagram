...
test("admin import apply-parsed route returns detected total pages in parse response", () => {
  const source = read(applyParsedRoutePath);

  assert.match(
    source,
    /parseDetectedTotalPages:\s*nextDiagnostics\?\.extraction\?\.detectedTotalPages\s*\?\?\s*null/,
    "Expected apply-parsed route success payload to include detected total PDF pages",
  );
});
