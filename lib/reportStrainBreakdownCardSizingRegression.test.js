...
test("strain section uses a shared three-column grid so breakdown and write-up cards have matching widths", () => {
  ...
  assert.match(
    html,
    /\.strain-cards-grid\s*\{[\s\S]*gap\s*:\s*10px[\s\S]*align-items\s*:\s*stretch/i,
    "Expected strain cards grid to use tighter spacing and stretch cards for a more even layout.",
  );

  assert.match(
    html,
    /\.strain-cards-pack\s+\.card\s*\{[\s\S]*height\s*:\s*100%/i,
    "Expected strain write-up cards to fill their grid row height for consistent visual alignment.",
  );
});
