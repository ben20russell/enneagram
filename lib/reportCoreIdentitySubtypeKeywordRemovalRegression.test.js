...
test("overview core identity motivation spacing matches standard label/value spacing", () => {
  const source = read(reportHtmlPath);
  const coreIdentityStart = source.indexOf('<div class="ct">Core Identity</div>');
  const profileCardStart = source.indexOf('<div class="ct">Your Enneagram Profile</div>', coreIdentityStart);
  const coreIdentityBlock =
  ...
  assert.doesNotMatch(
    coreIdentityBlock,
    /style="[^"]*margin:8px 0 0[^"]*"[\s\S]*id="motivationSummary"/,
    "Expected Motivation copy to avoid extra top margin that exceeds the card's standard label/value spacing.",
  );
  assert.match(
    coreIdentityBlock,
    /<div class="core-identity-motivation">[\s\S]*data-testid="core-identity-motivation-label"[\s\S]*class="core-identity-motivation-summary"[\s\S]*id="motivationSummary"/,
    "Expected Motivation to render in a dedicated spacing block with a consistent summary class.",
  );
  assert.match(
    source,
    /\.core-identity-motivation\{margin-top:12px\}/,
    "Expected Motivation block to maintain consistent row spacing from the Characteristics chips.",
  );
  assert.match(
    source,
    /\.core-identity-motivation-summary\{font-size:14px;color:var\(--text2\);line-height:1\.65;margin:0\}/,
    "Expected Motivation summary to keep standard label/value spacing and body copy styling.",
  );
});
