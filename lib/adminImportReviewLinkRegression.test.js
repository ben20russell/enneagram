...
test("admin import page includes a button to admin review", () => {
  ...
  assert.match(
    source,
    /function\s+handleOpenAdminReview\s*\(\)\s*\{[\s\S]*?router\.push\(\s*"\/admin#admin-review-section"\s*\)/,
    "Expected admin import page to navigate to the admin review section through a dedicated button handler.",
  );
