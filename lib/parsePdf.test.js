...
test("parsePdf injects canonical RAG context into OpenAI prompt content by default", async () => {
  setAzureOpenAiEnv();
  const originalFetch = global.fetch;
  const requestBodies = [];
  ...
  try {
    ...
    assert.match(userPromptContent, /Retrieved canonical Enneagram reference excerpts/i);
...
test("parsePdf skips canonical RAG context when disabled in parse options", async () => {
  setAzureOpenAiEnv();
  const originalFetch = global.fetch;
  const requestBodies = [];
  ...
  try {
    ...
    assert.equal(requestBodies.length, 1);
    const userPromptContent = String(requestBodies[0]?.messages?.[1]?.content || "");
    assert.doesNotMatch(userPromptContent, /Retrieved canonical Enneagram reference excerpts/i);
