...
test("parsePdf uses layout HTML + agentic OCR repair pipeline for PDF-first LLM parsing", async () => {
  ...
  try {
    ...
    assert.equal(result?._parseStatus, "complete");
    assert.equal(result?._parseDiagnostics?.extraction?.method, "local_text_only");
    assert.equal(requestBodies.length, 1, "Expected one text-only structured JSON call.");
...
test("parsePdf cleans OCR split/merged copy artifacts before structuring and in final payload", async () => {
  ...
  try {
    ...
    assert.equal(result?._parseStatus, "complete");
    assert.equal(requestBodies.length, 1, "Expected one text-only structuring call.");
