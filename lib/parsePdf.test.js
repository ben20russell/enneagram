...
test("parsePdf uses attached-style primary parsing only and does not emit validator diagnostics", async () => {
  setDocumentIntelligenceEnv();
  setAzureOpenAiEnv();
  ...
  try {
    ...
    assert.equal(result?._parseDiagnostics?.primaryValidation, undefined);
  } finally {
    DocumentAnalysisClient.prototype.beginAnalyzeDocument = originalBeginAnalyzeDocument;
    global.fetch = originalFetch;
  }
...
test("parsePdf does not retry or promote parser output when primary extraction has sparse critical fields", async () => {
  setDocumentIntelligenceEnv();
  setAzureOpenAiEnv();
