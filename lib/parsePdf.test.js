...
function setAzureOpenAiEnv() {
  ...
  process.env.AZURE_OPENAI_DEPLOYMENT_NAME = "gpt-5.6-sol";
...
test("parsePdf uses attached single-pass JSON parsing and maps core fields into legacy payload keys", async () => {
  ...
  try {
    ...
    assert.match(
      String(fetchBodies[0]?.messages?.[0]?.content || ""),
      /PUNCTUATION ENFORCEMENT/i,
    );
    assert.equal(result?._parseDiagnostics?.parserVersion, "attached-agentic-ocr-v1");
    ...
    assert.equal(result?.centerScores?.body, 100);
    assert.equal(result?.centerScores?.heart, 50);
    assert.equal(result?.centerScores?.head, 0);
...
test("parsePdf treats completeness as detected-page coverage instead of hard min-page fallback", async () => {
  setAzureOpenAiEnv();
  const originalFetch = global.fetch;
  ...
  try {
    ...
    assert.equal(result?._parseStatus, "complete");
    assert.equal(String(result?._parseDiagnostics?.incompleteReason || ""), "");
    assert.equal(result?._parseDiagnostics?.extraction?.pages, 1);
    assert.equal(result?._parseDiagnostics?.extraction?.detectedTotalPages, 1);
  } finally {
    global.fetch = originalFetch;
  }
});
...
test("parsePdf uses layout HTML + agentic OCR repair pipeline for PDF-first LLM parsing", async () => {
  setAzureOpenAiEnv();
  const originalFetch = global.fetch;
  const requestBodies = [];
  ...
  try {
    global.fetch = async function fetchMock(_url, init = {}) {
      requestBodies.push(JSON.parse(String(init?.body || "{}")));
      const currentRequest = requestBodies[requestBodies.length - 1];
      const systemPrompt = String(currentRequest?.messages?.[0]?.content || "");
      const isRepairCall = /you are an ocr repair agent/i.test(systemPrompt);
          ...
          async json() {
            return {
              choices: [
                {
                  message: {
                    content: "<h2>Main Type</h2><table><tr><td>Main Type</td><td>8</td></tr><tr><td>Dominant Instinct</td><td>SX</td></tr></table>",
                  },
                },
              ],
            };
    ...
    const { parsePdf } = await import(uniqueModuleUrl());
    const result = await parsePdf(Buffer.from("fake"), {
      reportId: "agentic-html-report",
      parseMinExpectedPages: 1,
      pageCountOverride: 1,
      layoutHtmlOverride: "<h2>Main Type</h2><table><tr><td>Main Type</td><td>8</td></tr><tr><td>Dominant Instinct</td><td>SX</td></tr></table>",
      enablePythonCrossCheck: false,
      allowLocalTextFallback: false,
    });

    assert.equal(result?._parseStatus, "complete");
    assert.equal(result?._parseDiagnostics?.extraction?.method, "agentic_ocr_repaired_html");
    assert.ok(requestBodies.length >= 2, "Expected one OCR-repair call plus one structured JSON call.");
    const structuringRequest = requestBodies.find((body) => /enneagram coach and data analyst/i.test(
      String(body?.messages?.[0]?.content || ""),
    ));
    const userPromptContent = String(structuringRequest?.messages?.[1]?.content || "");
    assert.match(
      userPromptContent,
      /extract data based on semantic alignment/i,
    );
    assert.match(userPromptContent, /<table>/i);
...
test("parsePdf cleans OCR split/merged copy artifacts before structuring and in final payload", async () => {
  setAzureOpenAiEnv();
  const originalFetch = global.fetch;
  const requestBodies = [];
  ...
  try {
    global.fetch = async function fetchMock(_url, init = {}) {
          ...
          async json() {
            return {
              choices: [
                {
                  message: {
                    content: "<p>Sum m ary placewhere longas</p>",
                  },
                },
              ],
            };
      ...
      const payload = mockAttachedJsonPayload();
      payload.core_profile.core_fear = "Sum m ary placewhere longas";
      payload.coaching_relationship.opportunities = [
        "Take action longas needed at placewhere trust is high.",
      ];
    ...
    const { parsePdf } = await import(uniqueModuleUrl());
    const result = await parsePdf(Buffer.from("fake"), {
      reportId: "ocr-artifact-cleanup-report",
      parseMinExpectedPages: 1,
      pageCountOverride: 1,
      layoutHtmlOverride: "<h2>Main Type</h2><table><tr><td>Main Type</td><td>8</td></tr></table>",
      enablePythonCrossCheck: false,
      allowLocalTextFallback: false,
    });

    assert.equal(result?._parseStatus, "complete");
    assert.ok(requestBodies.length >= 2, "Expected one repair call and one structuring call.");
    const structuringRequest = requestBodies.find((body) =>
      /expert enneagram coach and data analyst/i.test(String(body?.messages?.[0]?.content || "")),
    );
    const structuringUserPrompt = String(structuringRequest?.messages?.[1]?.content || "");
    assert.doesNotMatch(structuringUserPrompt, /\bSum m ary\b/i);
    assert.doesNotMatch(structuringUserPrompt, /\bplacewhere\b/i);
    assert.doesNotMatch(structuringUserPrompt, /\blongas\b/i);
    assert.equal(result?.coreFear, "Summary place where long as");
    assert.equal(/Sum m ary|placewhere|longas/i.test(JSON.stringify(result)), false);
  } finally {
    global.fetch = originalFetch;
  }
});

test("parsePdf does not require third-party parser env vars for PDF-first parsing", async () => {
  setAzureOpenAiEnv();
  const originalFetch = global.fetch;
  ...
  try {
    ...
    const { parsePdf } = await import(uniqueModuleUrl());
    const result = await parsePdf(Buffer.from("fake"), {
      reportId: "no-llama-env-required-report",
      parseMinExpectedPages: 1,
      pageCountOverride: 1,
      layoutHtmlOverride: "<h2>Main Type</h2><table><tr><td>Main Type</td><td>8</td></tr></table>",
      enablePythonCrossCheck: false,
...
test("parsePdf avoids fragmenting long unpunctuated narrative rows into chopped snippets", async () => {
  setAzureOpenAiEnv();
  const originalFetch = global.fetch;

  try {
    const longUnpunctuatedNarrative = [
      "Leaning forward to make a more personal connection with intense eye contact and elegant gestures while tracking group sentiment and interpersonal tone",
      "staying emotionally available under pressure and showing clear concern for relational impact without withdrawing from shared accountability",
    ].join(" ");

    global.fetch = async function fetchMock(_url, init = {}) {
      const body = JSON.parse(String(init?.body || "{}"));
      const payload = mockAttachedJsonPayload();
      const isStructuringCall = /expert enneagram coach and data analyst/i.test(
        String(body?.messages?.[0]?.content || ""),
      );
      if (isStructuringCall) {
        payload.communication_dynamics.body_language = longUnpunctuatedNarrative;
      }
      return {
        ok: true,
        async json() {
    ...
    const { parsePdf } = await import(uniqueModuleUrl());
    const result = await parsePdf(Buffer.from("fake"), {
      reportId: "long-unpunctuated-narrative-report",
      rawTextOverride: "Raw PDF text for long narrative split regression",
      parseMinExpectedPages: 1,
      pageCountOverride: 1,
    });

    const bodyLanguageRows = Array.isArray(result?.spreadsheetFocuses?.bodyLanguageRows)
      ? result.spreadsheetFocuses.bodyLanguageRows
      : [];
    assert.equal(bodyLanguageRows.length, 1);
    assert.match(String(bodyLanguageRows[0] || ""), /Leaning forward to make a more personal connection/i);
    assert.match(String(bodyLanguageRows[0] || ""), /relational impact without withdrawing from shared accountability/i);
  } finally {
    global.fetch = originalFetch;
  }
});
