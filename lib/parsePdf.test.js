...
test("parsePdf records primary-vs-validator diagnostics when primary pass is accepted", async () => {
  setDocumentIntelligenceEnv();
  setAzureOpenAiEnv();
  ...
  DocumentAnalysisClient.prototype.beginAnalyzeDocument = async function beginAnalyzeDocument() {
    return {
      async pollUntilDone() {
        return {
          paragraphs: [{ role: "content", content: "Balanced extraction sample text.", boundingRegions: [{ pageNumber: 32 }] }],
          tables: [],
          pages: Array.from({ length: 42 }, (_, idx) => ({ pageNumber: idx + 1 })),
        };
      },
  ...
  try {
    ...
    global.fetch = async function fetchMock() {
              ...
              choices: [
                {
                  message: {
                    content: JSON.stringify({
                      strain_interpretation: {
                        overall: "Overall strain remains moderate.",
                        vocational: "Vocational strain is manageable.",
                        environmental: "Environmental strain is low.",
                        physical: "Physical strain is medium.",
                        interpersonal: "Interpersonal strain is high.",
                        psychological: "Psychological strain is high.",
                        happiness: "Happiness improves with recovery.",
                      },
                      body_language: ["Steady posture in calm states."],
                      feedback_guide: {
                        type_1: ["Tip 1"],
                        type_2: ["Tip 2"],
                        type_3: ["Tip 3"],
                      ...
                      decision_framework: {
                        dominant_center_impact: ["Dominant center impacts framing."],
                        making_decisions: ["Clarify decision criteria."],
                        receiving_decisions: ["Ask for context early."],
                        strain_impact: ["Strain narrows options."],
                      },
                      strategic_leadership: {
                        visioning: "Visioning text",
                        strategic_thinking: "Strategic text",
                        alignment: "Alignment text",
                        change_management: "Change text",
                      },
                      team_dynamics: {
                        interdependence_and_role: "Interdependence text",
                        forming: ["Forming text"],
                        storming: ["Storming text"],
                        norming: ["Norming text"],
                        performing: ["Performing text"],
                      },
                      coaching_relationship: ["Coaching relationship text with specifics."],
                      development_exercises: {
                        core_type: ["Core text"],
                        subtype: ["Subtype text"],
                        centers: ["Centers text"],
                        integration: ["Integration text"],
                        strain: ["Strain text"],
                        conflict: ["Conflict text"],
                        management: ["Management text"],
                        strategic_leadership: ["Strategic text"],
                      },
                    }),
                  },
                },
      ...
      throw new Error(`Unexpected fetch call count: ${fetchCount}`);
    };

    const { parsePdf } = await import(uniqueModuleUrl());
    const result = await parsePdf(Buffer.from("fake"), "primary-accepted-report");

    assert.equal(fetchCount, 2);
    assert.equal(result?._parseDiagnostics?.primaryValidation?.selectedSource, "primary");
    assert.equal(result?._parseDiagnostics?.primaryValidation?.retryAttempted, false);
    assert.equal(
      Number.isFinite(Number(result?._parseDiagnostics?.primaryValidation?.diffRate)),
      true,
    );
  } finally {
    DocumentAnalysisClient.prototype.beginAnalyzeDocument = originalBeginAnalyzeDocument;
...
test("parsePdf retries primary extraction once and promotes validator output on persistent hard fail", async () => {
  setDocumentIntelligenceEnv();
  setAzureOpenAiEnv();
  ...
  DocumentAnalysisClient.prototype.beginAnalyzeDocument = async function beginAnalyzeDocument() {
    return {
      async pollUntilDone() {
        return {
          paragraphs: [{ role: "content", content: "Retry and promotion sample text.", boundingRegions: [{ pageNumber: 32 }] }],
          tables: [],
          pages: Array.from({ length: 42 }, (_, idx) => ({ pageNumber: idx + 1 })),
        };
      },
  ...
  try {
    ...
    const { parsePdf } = await import(uniqueModuleUrl());
    const result = await parsePdf(Buffer.from("fake"), "primary-retry-promotion-report");

    assert.equal(fetchCount, 3);
    assert.equal(result?._parseDiagnostics?.primaryValidation?.retryAttempted, true);
    assert.equal(result?._parseDiagnostics?.primaryValidation?.retrySucceeded, false);
    assert.equal(result?._parseDiagnostics?.primaryValidation?.selectedSource, "validator");
    assert.equal(
      Array.isArray(result?._parseDiagnostics?.primaryValidation?.hardFailReasons),
      true,
    );
    assert.equal(
      result._parseDiagnostics.primaryValidation.hardFailReasons.length > 0,
      true,
    );
  } finally {
    DocumentAnalysisClient.prototype.beginAnalyzeDocument = originalBeginAnalyzeDocument;
