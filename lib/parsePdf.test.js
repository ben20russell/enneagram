...
test("parsePdf module throws when Document Intelligence env vars are missing", async () => {
...
test("parsePdf returns complete status when Document Intelligence returns pages", async () => {
  ...
  DocumentAnalysisClient.prototype.beginAnalyzeDocument = async function beginAnalyzeDocument(modelName, pdfBuffer) {
    calls.push({ modelName, pdfBuffer });
    return {
      async pollUntilDone() {
        return {
          paragraphs: [
