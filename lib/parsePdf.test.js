...
test("parsePdf uses python docling markdown payload directly for LLM parsing when available", async () => {
  setAzureOpenAiEnv();
  setAzureDocIntelEnv();
  const originalFetch = global.fetch;
  const requestBodies = [];
  ...
  try {
    ...
    const { parsePdf } = await import(uniqueModuleUrl());
    const result = await parsePdf(Buffer.from("fake"), {
      reportId: "python-docling-markdown-report",
      parseMinExpectedPages: 1,
      pageCountOverride: 1,
      pythonVerificationOverride: {
        source: "docling_markdown",
        markdown: "# iEQ9 Report\n\n## Main Type\n8\n\n## Dominant Instinct\nSX",
      },
    });

    assert.equal(result?._parseStatus, "complete");
    assert.equal(result?._parseDiagnostics?.extraction?.method, "python_docling_markdown");
    const userPromptContent = String(requestBodies?.[0]?.messages?.[1]?.content || "");
    assert.match(
      userPromptContent,
...
test("parsePdf returns failed diagnostics when Document Intelligence env is missing for PDF-first parsing", async () => {
  setAzureOpenAiEnv();
  const previousDocIntelEndpoint = process.env.AZURE_DOCUMENT_INTELLIGENCE_ENDPOINT;
  const previousDocIntelKey = process.env.AZURE_DOCUMENT_INTELLIGENCE_KEY;
  ...
  try {
    ...
    assert.match(
      ...
      /Missing Azure environment variables:.*AZURE_DOCUMENT_INTELLIGENCE_ENDPOINT/i,
    );
  } finally {
    if (typeof previousDocIntelEndpoint === "undefined") delete process.env.AZURE_DOCUMENT_INTELLIGENCE_ENDPOINT;
    else process.env.AZURE_DOCUMENT_INTELLIGENCE_ENDPOINT = previousDocIntelEndpoint;
    if (typeof previousDocIntelKey === "undefined") delete process.env.AZURE_DOCUMENT_INTELLIGENCE_KEY;
    else process.env.AZURE_DOCUMENT_INTELLIGENCE_KEY = previousDocIntelKey;
  }
});
