...
test("parsePdf section pass includes PRO keys and feedback-grid guidance", async () => {
  ...
  try {
    ...
    assert.ok(observedSectionSchema?.properties?.proSections?.properties?.strategic_leadership);
    assert.ok(observedSectionSchema?.properties?.proSections?.properties?.team_dynamics);
    assert.ok(observedSectionSchema?.properties?.proSections?.properties?.decision_framework);
    assert.ok(observedSectionSchema?.properties?.proSections?.properties?.coaching_relationship);
  } finally {
    process.env.PDF_PARSE_MIN_PAGES = originalMinPages;
  }
});
