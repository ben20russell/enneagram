...
test("profile wheel role label geometry clears the wheel across all nine canonical type layouts", () => {
  ...
  const roleLabelConfig = [
    { key: "release", label: "RELEASE", angleOffset: -6, radialOffset: 0, xNudge: 8, yNudge: 3 },
    { key: "stretch", label: "STRETCH", angleOffset: 6, radialOffset: 6, xNudge: 8, yNudge: 3 },
  ];
  const cx = 292;
  const cy = 166;
...
test("profile wheel badge only renders instinct and type without subtype keyword copy", () => {
  const html = read(reportHtmlPath);
  const script = read(reportJsPath);

  assert.doesNotMatch(
    html,
    /id="profileWheelBadgeKeyword"/,
    "Expected the profile wheel badge to remove the third keyword/subtype line.",
  );

  assert.match(
    html,
    /id="profileWheelBadgeInstinct"[\s\S]*id="profileWheelBadgeType"/,
    "Expected the profile wheel badge to keep instinct and type values.",
  );

  assert.doesNotMatch(
    script,
    /setText\('profileWheelBadgeKeyword'/,
    "Expected report rendering to stop hydrating a removed profile wheel keyword node.",
  );
});

test("profile wheel badge vertically centers instinct and type copy inside the circle", () => {
  const html = read(reportHtmlPath);

  assert.match(
    html,
    /\.profile-wheel-badge\{[\s\S]*justify-content:center[\s\S]*padding-top:0[^}]*\}/i,
    "Expected profile wheel badge content to be vertically centered with no top padding offset.",
  );

  assert.match(
    html,
    /@media\(max-width:700px\)\{[\s\S]*\.profile-wheel-badge\{[\s\S]*justify-content:center[\s\S]*padding-top:0[^}]*\}/i,
    "Expected mobile profile wheel badge to keep instinct/type copy vertically centered.",
  );
});
