...
function runExtractTypeFromPages(pages) {
  const output = execFileSync(
    "python3",
    [
      "-c",
...
detected, source = module.extract_type_from_pages(pages, preferred_page_number=6)
print(json.dumps({"detected": detected, "source": source}))
      `.trim(),
    ],
    {
...
test("extract_report_pdf prioritizes page 6 resonance sentence for main type detection", () => {
  const pages = [
    "Main Type # 3 appears in front-matter legend.",
    "Type 1 and Type 2 comparison overview.",
    "Nothing useful on this page.",
    "Main Type # 4 appears in sample table.",
    "Background section with no definitive identity line.",
    "Ben, you resonate with the Enneagram type 8 which is also known as the Active Controller.",
    "Appendix references Type 7 examples.",
  ];

  const detected = runExtractTypeFromPages(pages);

  assert.equal(detected?.detected, "8");
  assert.equal(detected?.source, "page6:resonanceSentence");
});
