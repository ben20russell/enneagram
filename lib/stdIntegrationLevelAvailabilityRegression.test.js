...
test("dashboard rendering hides integration UI for reports that do not support integration level", () => {
  ...
  assert.match(
    jsSource,
    /setIntegrationUiVisibility\(\s*!isStdReportLayout\s*&&\s*supportsIntegrationLevel\s*\)/,
    "Expected report render flow to hide integration UI when integration is unavailable or the report uses STD layout.",
  );
});

test("dashboard rendering hides strain tab and section for reports without strain profile support", () => {
  const htmlSource = read(reportHtmlPath);
  const jsSource = read(reportJsPath);
  ...
  assert.match(
    htmlSource,
    /id="sec-strain"/,
    "Expected strain section to expose a dedicated section id for conditional visibility handling.",
  );

  assert.match(
    jsSource,
    /function\s+setStrainUiVisibility\s*\(/,
    "Expected dashboard script to define a reusable strain-visibility toggler.",
  );

  assert.match(
    jsSource,
    /const\s+supportsStrainProfile\s*=\s*REPORT\?\.supportsStrainProfile\s*!==\s*false/,
    "Expected report render flow to read strain-profile support from hydrated report state.",
  );

  assert.match(
    jsSource,
    /setStrainUiVisibility\(\s*!isStdReportLayout\s*&&\s*supportsStrainProfile\s*\)/,
    "Expected report render flow to hide strain UI when strain profile is unavailable or the report uses STD layout.",
  );
});

test("dashboard rendering removes PRO-only leadership and communication sections for STD reports", () => {
  const htmlSource = read(reportHtmlPath);
  const jsSource = read(reportJsPath);
  ...
  assert.match(
    htmlSource,
    /id="sec-leadership"/,
    "Expected leadership section id to remain available for runtime visibility control.",
  );

  assert.match(
    htmlSource,
    /id="sec-communication"/,
    "Expected communication section id to remain available for runtime visibility control.",
  );

  assert.match(
    jsSource,
    /function\s+setDashboardSectionVisibility\s*\(/,
    "Expected dashboard script to expose a shared section visibility helper.",
  );

  assert.match(
    jsSource,
    /const\s+isStdReportLayout\s*=\s*!isExampleMode\s*&&\s*String\(REPORT\?\.reportType\s*\|\|\s*""\)\.trim\(\)\.toUpperCase\(\)\s*===\s*"STD"/,
    "Expected report render flow to derive STD layout mode from reportType for assigned/client reports.",
  );

  assert.match(
    jsSource,
    /setDashboardSectionVisibility\(\s*"leadership"\s*,\s*!isStdReportLayout\s*\)/,
    "Expected leadership section visibility to be disabled for STD report layouts.",
  );

  assert.match(
    jsSource,
    /setDashboardSectionVisibility\(\s*"communication"\s*,\s*!isStdReportLayout\s*\)/,
    "Expected communication section visibility to be disabled for STD report layouts.",
  );
});
